package com.mineradio.app.player.impl
...
import com.mineradio.app.player.api.PlayerAction
import com.mineradio.app.player.impl.dsp.EqualizerAudioProcessor
import com.mineradio.app.player.impl.dsp.FFTAudioProcessor
import com.mineradio.app.player.impl.dsp.FFTAudioProcessorWrapper
import com.mineradio.app.player.impl.dsp.ReverbAudioProcessor
import com.mineradio.app.player.impl.dsp.SurroundAudioProcessor
import com.mineradio.app.player.impl.utils.MediaLibrary
import com.mineradio.app.player.impl.utils.PlayerKVUtils
import com.mineradio.app.player.impl.utils.toMediaItem
...
class SpicaPlayer(
    ...
    private val _surroundProcessor = SurroundAudioProcessor()
     ...
     *
     * ★ 恢复上次播放进度：读取保存的 mediaId + position，seekTo 对应位置
     */
    override fun init() {
        ...
        launch(Dispatchers.Main) {
            try {
                ...
                // ★ 恢复上次播放进度：查找保存的 mediaId 在列表中的位置
                val savedMediaId = playerKVUtils.getSavedMediaId()
                val savedPosition = playerKVUtils.getSavedPositionMs()
                if (!savedMediaId.isNullOrBlank()) {
                    val savedIndex = items.indexOfFirst { it.mediaId == savedMediaId }
                    if (savedIndex >= 0 && savedIndex < items.size) {
                        // seekTo(index, position) 会将播放器定位到指定歌曲的指定位置
                        // ★ 即使 position 为 0 也要 seekTo，确保恢复到正确的歌曲（而非列表第一首）
                        val positionToRestore = savedPosition.coerceAtLeast(0L)
                        browser.seekTo(savedIndex, positionToRestore)
                        Timber.tag(TAG).d("Restored playback: mediaId=$savedMediaId, position=$positionToRestore, index=$savedIndex")
                        // 更新当前媒体项状态
                        _currentMediaItem.value = items[savedIndex]
                    } else {
                        // 保存的 mediaId 不在列表中，从头开始
                        Timber.tag(TAG).d("Saved mediaId not in playlist, starting fresh")
                    }
                }

                browser.prepare()

                // ★ 启动周期性保存播放进度（每 5 秒保存一次，防止异常退出丢失进度）
                //   重连时先取消旧任务，避免多个保存循环并存
                positionSaveJob?.cancel()
                positionSaveJob = launch(Dispatchers.IO) {
                    while (true) {
                        kotlinx.coroutines.delay(5000L)
                        try {
                            val currentBrowser = browserInstance
                            val mediaItem = _currentMediaItem.value
                            if (currentBrowser != null && mediaItem != null) {
                                // ★ 修复：MediaController.getCurrentPosition 必须在主线程调用
                                //   原代码直接在 IO 线程调用，每 5 秒抛 IllegalStateException
                                val pos = withContext(Dispatchers.Main) { currentBrowser.currentPosition }
                                val dur = withContext(Dispatchers.Main) { currentBrowser.duration }
                                // 仅在有效范围内保存（避免保存 0 或负值，避免保存已结束的进度）
                                if (pos > 0L && (dur <= 0L || pos < dur)) {
                                    playerKVUtils.savePlaybackPosition(mediaItem.mediaId, pos)
                                }
                            }
                        } catch (e: Exception) {
                            Timber.tag(TAG).w(e, "Periodic position save failed")
                        }
                    }
                }
            } catch (e: Exception) {
    ...
    override fun onMediaItemTransition(
        ...
        // ★ 切歌时保存新的 mediaId（位置为0），防止用户在切歌后立即退出时恢复到旧歌曲
        if (mediaItem != null) {
            launch(Dispatchers.IO) {
                try {
                    playerKVUtils.savePlaybackPosition(mediaItem.mediaId, 0L)
                } catch (e: Exception) {
                    Timber.tag(TAG).w(e, "Failed to save mediaId on transition")
                }
            }
        }
        ...
        // ★ 切歌后重新应用环绕模式，防止 ExoPlayer 重置音频处理器链导致 3D/360 环绕失效
        //   即使 SurroundAudioProcessor.reset() 已不重置 mode，这里作为双保险
        val currentMode = _surroundProcessor.getMode()
        if (currentMode != SurroundAudioProcessor.MODE_OFF) {
            _surroundProcessor.setMode(currentMode)
        }
    ...
    override fun setSurroundMode(mode: Int) {
        _surroundProcessor.setMode(mode)
    }
