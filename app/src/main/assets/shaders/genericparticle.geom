...
void main() {
	...
	vec3 uvOffsets = vec3(1.0, 1.0, 0.0);
	...
	OUT.Append(CreateParticleVertex(vec2(0.0, 0.0), spriteBlend, uvs,                  textureRatio, IN[0], right, up));
	OUT.Append(CreateParticleVertex(vec2(0.0, 1.0), spriteBlend, uvs + uvOffsets.zyzy, textureRatio, IN[0], right, up));
	OUT.Append(CreateParticleVertex(vec2(1.0, 0.0), spriteBlend, uvs + uvOffsets.xzxz, textureRatio, IN[0], right, up));
	OUT.Append(CreateParticleVertex(vec2(1.0, 1.0), spriteBlend, uvs + uvOffsets.xyxy, textureRatio, IN[0], right, up));
}
