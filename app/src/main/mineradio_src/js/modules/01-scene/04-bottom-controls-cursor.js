...
var TOUCH_AUTO_HIDE_DELAY = 3000;
...
function scheduleControlsHide(delay) {
  ...
  // 触摸设备：最少使用 TOUCH_AUTO_HIDE_DELAY（3秒）
  if (IS_TOUCH_DEVICE && requestedDelay < TOUCH_AUTO_HIDE_DELAY) requestedDelay = TOUCH_AUTO_HIDE_DELAY;
...
var _autoHideRectCache = null;
var _autoHideRectCacheAt = 0;
function _cachedAutoHideRects() {
  var now = performance.now();
  if (_autoHideRectCache && now - _autoHideRectCacheAt < 80) return _autoHideRectCache;
  var bar = document.getElementById('bottom-bar');
  var handle = document.getElementById('bottom-handle');
  ...
  _autoHideRectCache = {
    ...
    mini: mini ? mini.getBoundingClientRect() : null
  };
  _autoHideRectCacheAt = now;
  return _autoHideRectCache;
}
function updateControlsAutoHideFromPointer(x, y) {
  ...
  var rects = _cachedAutoHideRects();
  var hr = rects.handle;
  var overHandle = hr && x >= hr.left - 18 && x <= hr.right + 18 && y >= hr.top - 12 && y <= hr.bottom + 14;
  var overBar = rects.bar && x >= rects.bar.left - 18 && x <= rects.bar.right + 18 && y >= rects.bar.top - 18 && y <= rects.bar.bottom + 14;
  var miniRect = rects.mini;
...
(function initControlsAutoHide() {
  ...
  if (IS_TOUCH_DEVICE) {
    var touchHandler = function (e) {
      ...
      var tappedOnBar = touchX >= barRect.left && touchX <= barRect.right && touchY >= barRect.top && touchY <= barRect.bottom;
      var tappedOnHandle = handleRect && touchX >= handleRect.left && touchX <= handleRect.right && touchY >= handleRect.top && touchY <= handleRect.bottom;
      if (tappedOnBar || tappedOnHandle) {
        // 点击控制栏/手柄：唤出，3秒后隐藏
        if (!bar.classList.contains('visible')) bar.classList.add('visible');
        setControlsHidden(false);
        wakeBottomHandle();
        if (controlsHideTimer) { clearTimeout(controlsHideTimer); controlsHideTimer = null; }
        if (controlsAutoHide) scheduleControlsHide(TOUCH_AUTO_HIDE_DELAY);
      } else {
        // 点击外部：若控制栏已隐藏则唤出（沉浸模式下控制条隐藏后需能重新唤出），
        // 若正可见则 3 秒后自动隐藏
        if (bar.classList.contains('visible')) {
          if (!bar.classList.contains('soft-hidden')) {
            if (controlsHideTimer) { clearTimeout(controlsHideTimer); controlsHideTimer = null; }
            if (controlsAutoHide) scheduleControlsHide(TOUCH_AUTO_HIDE_DELAY);
          } else {
            setControlsHidden(false);
