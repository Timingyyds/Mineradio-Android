...
function refreshMemorySnapshot(force) {
  if (!window.desktopWindow) {
    // ★ Android 手机版：无桌面内存桥接，走原生 KeepApp 读取手机运行内存（仅展示，不做系统级释放）
    if (window.KeepApp && typeof window.KeepApp.getDeviceMemoryInfo === 'function') {
      try {
        var info = JSON.parse(window.KeepApp.getDeviceMemoryInfo() || '{}');
        if (info && info.ok) {
          updateMemoryStatusText('手机运行内存 ' + info.usedMB + '/' + info.totalMB + ' MB (' + info.usedPercent + '%)，可用 ' + info.freeMB + ' MB');
        } else {
          updateMemoryStatusText('手机内存读取失败');
        }
      } catch (e) {
        updateMemoryStatusText('手机内存读取失败');
      }
...
function updateMemoryControls() {
  ensureMemoryFxDefaults();
  // ★ Android 手机版：隐藏仅桌面可用的系统级 Mem Reduct 控件，避免显示不可用按钮
  var androidOnly = !window.desktopWindow;
  ['memory-mask-seg', 't-memoryAutoSystemTrim', 't-memorySystemAutoElevate', 'fx-memory-interval', 'fx-memory-threshold'].forEach(function (id) {
    var el = document.getElementById(id);
    if (el) el.style.display = androidOnly ? 'none' : '';
  });
  var actionRow = document.querySelector('.memory-action-row');
  if (actionRow) actionRow.style.display = androidOnly ? 'none' : '';
  var statusSub = document.getElementById('memory-status-sub');
  if (statusSub) statusSub.textContent = androidOnly
    ? '显示手机当前运行内存占用；系统级 Mem Reduct 仅桌面版可用。'
    : '后台先压缩播放器进程；系统级释放可清工作集、修改页、待机页';
  [
