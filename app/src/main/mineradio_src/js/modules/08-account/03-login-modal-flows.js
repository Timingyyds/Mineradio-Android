...
function loginProviderOfficialModeText(provider) {
  ...
  if (provider === 'qishui') return { title: '官网', sub: '弹出汽水音乐官方窗口' };
  if (provider === 'kugou') return { title: '官网', sub: '弹出酷狗官方窗口' };
  if (provider === 'netease') return { title: '扫码', sub: '使用内置二维码登录' };
...
function renderLoginWorkflowEdges(tempPoint) {
  ...
  if (!graph || !svg) return;
  // 使用 getBoundingClientRect 获取视觉尺寸,与 workflowPointForPort 的坐标系一致
  // 避免 CSS zoom/transform 导致 clientWidth(布局像素) 与 getBoundingClientRect(视觉像素) 不匹配
  var rootRect = graph.getBoundingClientRect();
  var w = Math.max(1, rootRect.width || 1);
  var h = Math.max(1, rootRect.height || 1);
...
function selectLoginProviderNode(provider) {
  ...
  provider = normalizeLoginProviderKey(provider);
  var prevProvider = loginProvider;
  setLoginProvider(provider, true);
  // 点击平台栏时总是打开下方登录面板
  var drawerOpen = true;
  setLoginAuthDrawerOpen(drawerOpen);
  // 切换平台时立即清空旧二维码图片,避免显示上一个平台的二维码
  if (prevProvider !== provider) {
    var oldQrImg = document.getElementById('qr-img');
    if (oldQrImg) { oldQrImg.src = ''; oldQrImg.removeAttribute('src'); oldQrImg.style.display = ''; }
    var oldQrStatus = document.getElementById('qr-status');
    if (oldQrStatus) { oldQrStatus.textContent = ''; oldQrStatus.className = ''; }
    try { stopQrPoll(); } catch (e) { }
    qrKey = null;
  }
  updateLoginProviderUi();
  // 汽水音乐: 不再内置二维码,登录走官方网页窗口
}
...
function updateLoginProviderUi() {
  ...
  if (desc) desc.innerHTML = isQQ
    ...
    : (isQishui
      ? '打开 <b>汽水音乐官方网页登录窗口</b> 扫码，成功后会自动同步账号会话。'
    : '使用 <b>网易云音乐 App</b> 扫码，可同步歌单、红心与播客。'));
  var manualCookieOpen = isManualCookieOpenForProvider(loginProvider);
  // 移动端检测: 非桌面环境视为移动端
  var isMobileEnv = !!(window.desktopWindow && !window.desktopWindow.isDesktop);
  // 移动端 QQ/酷狗: 不强制 Cookie 导入,保留"打开官网"按钮供用户扫码
  if (shell) {
    // QQ/酷狗/汽水: 桌面端或移动端都使用 web-login-preview(隐藏二维码区,显示打开官方窗口按钮)
    // 网易云: 始终使用内置二维码
    var useWebPreview = isQQ || isKugou || isQishui;
    shell.classList.toggle('web-login-preview', useWebPreview);
    shell.classList.toggle('qq-preview', isQQ);
    // 进入 web 预览模式时,确保旧二维码图片被清空
    if (useWebPreview) {
      var clearImg = document.getElementById('qr-img');
      if (clearImg && clearImg.src) { clearImg.src = ''; clearImg.removeAttribute('src'); }
    }
  }
  ...
  if (qqCard) {
    ...
    if (cardLabel) cardLabel.textContent = isQQ
      ...
      : (isKugou ? (kugouWebLoginBusy ? '等待登录确认' : '打开官方登录窗口') : (isQishui ? (qishuiOAuthBusy ? '打开中…' : '打开官方扫码窗口') : (neteaseWebLoginBusy ? '等待扫码确认' : '刷新二维码')));
  }
  if (st) {
  ...
  if (refreshBtn) {
    ...
    refreshBtn.textContent = isQishui ? (qishuiOAuthBusy ? '打开中…' : '扫码登录') : (isQQ ? (qqWebLoginBusy ? '等待扫码…' : (qqNeedsAuthRefresh ? '重新授权' : (qqNeedsMembershipSync ? '同步会员' : (qqLoginStatus.loggedIn ? '刷新状态' : '扫码登录')))) : (isKugou ? (kugouWebLoginBusy ? '等待登录…' : '登录') : (neteaseWebLoginBusy ? '等待扫码…' : '刷新二维码')));
...
async function refreshQr() {
  ...
  if (loginProvider === 'qishui') {
    ...
    if (qishuiImg) { qishuiImg.src = ''; qishuiImg.removeAttribute('src'); }
    if (qishuiStatus) {
      qishuiStatus.textContent = '汽水音乐不使用内置二维码，点击“打开官方扫码窗口”在官网登录';
...
function startQrPoll() {
  ...
  if (loginProvider === 'qishui') {
    ...
    qrPollTimer = setTimeout(function () { pollQishuiQr(generation); }, 2000);
    return;
  }
...
// ============================================================
// 汽水音乐安全签名桥接: 通过 Android 隐藏 WebView 的 bdms.js 生成 a_bogus 签名
// Java 侧在签名 WebView 中执行 __qishuiRequest, bdms.js 自动注入 a_bogus
// 完成后通过 AndroidSignerCallback 回调 Java, Java 再通过 evaluateJavascript
// 调用 window.__qishuiSignCallback / __qishuiSignError 通知前端
// ============================================================
var qishuiSignCallbacks = {};
var qishuiSignCallbackId = 0;
window.__qishuiSignCallback = function (callbackId, result) {
  var cb = qishuiSignCallbacks[callbackId];
  if (cb) { delete qishuiSignCallbacks[callbackId]; cb.resolve(result); }
};
window.__qishuiSignError = function (callbackId, error) {
  var cb = qishuiSignCallbacks[callbackId];
  if (cb) { delete qishuiSignCallbacks[callbackId]; cb.reject(new Error(String(error))); }
};
function qishuiSignedRequest(payload) {
  return new Promise(function (resolve, reject) {
    if (!window.AndroidExternal || typeof window.AndroidExternal.signedRequest !== 'function') {
      reject(new Error('签名组件不可用'));
      return;
    }
    var id = String(++qishuiSignCallbackId);
    qishuiSignCallbacks[id] = { resolve: resolve, reject: reject };
    try {
      window.AndroidExternal.signedRequest(JSON.stringify(payload), id);
    } catch (e) {
      delete qishuiSignCallbacks[id];
      reject(e);
    }
  });
}
// ============================================================
// 汽水音乐二次验证(MFA/2046)处理
// 后端返回 needMfa: true 时,通过 Android 桥接显示验证面板
// 用户完成验证后通过 __qishuiMfaHandler 回调处理结果
// ============================================================
var qishuiMfaPendingResolve = null;
var qishuiMfaPendingReject = null;

window.__qishuiMfaHandler = function (mfaResult) {
  console.log('[QishuiMFA] __qishuiMfaHandler invoked', mfaResult && mfaResult.status);
  if (qishuiMfaPendingResolve) {
    var resolve = qishuiMfaPendingResolve;
    var reject = qishuiMfaPendingReject;
    qishuiMfaPendingResolve = null;
    qishuiMfaPendingReject = null;
    if (mfaResult && mfaResult.status) {
      resolve(mfaResult);
    } else {
      reject(new Error((mfaResult && mfaResult.message) || '二次验证失败'));
    }
  }
};

window.onQishuiSignerReady = function () {
  console.log('[QishuiMFA] onQishuiSignerReady called');
};
...
async function pollQishuiQr(generation) {
  ...
  var nextDelay = 4500;
  // 等待签名桥接就绪(桌面版流程:所有请求必须通过签名组件发出,带 a_bogus 避免频率限制)
  if (!(window.AndroidExternal && typeof window.AndroidExternal.isSignerReady === 'function' && window.AndroidExternal.isSignerReady())) {
    if (statusEl) { statusEl.textContent = '安全组件加载中,请稍候…'; statusEl.className = 'preview'; }
    scheduleQishuiQrPoll(generation, 2000);
    return;
  }
  qishuiQrPollBusy = true;
  try {
    // 从服务器获取请求 payload(不直接发起 HTTP 请求,与桌面版流程一致)
    ...
    // 通过签名 WebView 桥接发出请求(带 a_bogus),并回传签名后的响应给后端处理
    if (result && result.needSign && result.payload) {
      if (window.AndroidExternal && typeof window.AndroidExternal.isSignerReady === 'function' && window.AndroidExternal.isSignerReady()) {
        try {
          if (statusEl) { statusEl.textContent = '正在安全签名检查…'; statusEl.className = 'preview'; }
          var signedResponse = await qishuiSignedRequest(result.payload);
          // 将签名后的响应回传给后端处理
          result = await apiJson('/api/qishui/login/check/result', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: qrKey, responseBody: signedResponse.body || '' })
          });
          if (generation !== qishuiQrPollGeneration || loginProvider !== 'qishui') return;
        } catch (signErr) {
          console.warn('Qishui sign bridge failed:', signErr);
          nextDelay = 8000;
          if (statusEl) { statusEl.textContent = '签名检查失败,稍后重试…'; statusEl.className = 'fail'; }
          return;
        }
      } else {
        // 签名组件未就绪,延迟重试
    ...
    // 处理二次验证(MFA/2046): 需要用户完成额外验证(如手机验证)
    if (result && result.needMfa && result.mfaDecision) {
      if (window.AndroidExternal && typeof window.AndroidExternal.showSignerMfa === 'function') {
        try {
          if (statusEl) { statusEl.textContent = '需要二次验证,正在打开验证面板…'; statusEl.className = 'preview'; }
          // 通过 Android 桥接显示 MFA 验证面板
          window.AndroidExternal.showSignerMfa(JSON.stringify(result.mfaDecision));
          if (statusEl) { statusEl.textContent = '请在验证面板中完成二次验证…'; statusEl.className = 'preview'; }
          // 等待 MFA 验证完成(通过 __qishuiMfaHandler 回调)
          var mfaResult = await new Promise(function (resolve, reject) {
            qishuiMfaPendingResolve = resolve;
            qishuiMfaPendingReject = reject;
            // 超时保护: 5分钟后自动取消
            setTimeout(function () {
              if (qishuiMfaPendingResolve) {
                qishuiMfaPendingResolve = null;
          ...
          if (generation !== qishuiQrPollGeneration || loginProvider !== 'qishui') return;
          // MFA 验证成功,重新发送 check_qrconnect 请求(携带 bizParams)
          if (statusEl) { statusEl.textContent = '二次验证成功,继续检查登录状态…'; statusEl.className = 'preview'; }
          var resendResult = await apiJson('/api/qishui/login/check/result', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              token: qrKey,
              responseBody: mfaResult.body || '',
              bizParams: result.bizParams || {}
            })
          });
          if (generation !== qishuiQrPollGeneration || loginProvider !== 'qishui') return;
          // 用重发结果替换当前 result,继续后续登录状态检查
          result = resendResult;
        } catch (mfaErr) {
          console.warn('[QishuiMFA] MFA verification failed:', mfaErr);
          stopQrPoll();
          if (statusEl) {
            statusEl.textContent = '二次验证失败: ' + (mfaErr && mfaErr.message ? mfaErr.message : '用户取消或超时');
            statusEl.className = 'fail';
          }
          return;
        }
      } else {
        // Android 桥接不可用,无法进行 MFA 验证
        stopQrPoll();
        if (statusEl) {
          statusEl.textContent = '当前环境不支持二次验证,请用其他平台登录';
          statusEl.className = 'fail';
        }
        return;
      }
    ...
    if (code === 7 || qrStatus === 'rate_limited') {
      nextDelay = Math.min(Number(result && result.retryAfterMs) || 15000, 15000);
      if (statusEl) {
        statusEl.textContent = '请求较频繁，15秒后自动继续检查…';
...
async function openQQWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openQQMusicLogin !== 'function') {
    // 移动端: 打开 QQ 音乐官网登录页(应用内浏览器全屏),扫码登录后自动获取Cookie并返回
    qqManualCookieOpen = true;
    updateLoginProviderUi();
    if (statusEl) { statusEl.innerHTML = '已打开 QQ 音乐官网,请扫码登录,登录成功后将自动返回并同步。'; statusEl.className = 'preview'; }
    // 注册 Cookie 回调: 应用内浏览器检测到登录后自动回传 Cookie
    window.onInAppBrowserCookie = function (cookie) {
      if (!cookie) { if (statusEl) { statusEl.textContent = '未获取到 Cookie,请先在官网登录'; statusEl.className = 'fail'; } return; }
      var cookieInput = document.getElementById('qq-cookie-input');
      if (cookieInput) cookieInput.value = cookie;
      if (statusEl) { statusEl.textContent = '已获取 Cookie,正在同步...'; statusEl.className = 'preview'; }
      // 自动提交 Cookie
      var saveBtn = document.getElementById('qq-cookie-save-btn');
      if (saveBtn) saveBtn.click();
    };
    try { if (window.AndroidExternal && window.AndroidExternal.open) window.AndroidExternal.open('https://y.qq.com/portal/profile.html'); else window.open('https://y.qq.com/portal/profile.html', '_blank'); } catch (e) { }
    return;
  }
...
async function openKugouWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openKugouMusicLogin !== 'function') {
    // 移动端: 打开酷狗音乐官网登录页(应用内浏览器全屏),登录后自动获取Cookie并返回
    kugouManualCookieOpen = true;
    updateLoginProviderUi();
    if (statusEl) { statusEl.innerHTML = '已打开酷狗音乐官网,请登录,登录成功后将自动返回并同步。'; statusEl.className = 'preview'; }
    // 注册 Cookie 回调: 应用内浏览器回传 Cookie 时自动填充并提交
    window.onInAppBrowserCookie = function (cookie) {
      if (!cookie) { if (statusEl) { statusEl.textContent = '未获取到 Cookie,请先在官网登录'; statusEl.className = 'fail'; } return; }
      var cookieInput = document.getElementById('qq-cookie-input');
      if (cookieInput) cookieInput.value = cookie;
    ...
    try { if (window.AndroidExternal && window.AndroidExternal.open) window.AndroidExternal.open('https://www.kugou.com/'); else window.open('https://www.kugou.com/', '_blank'); } catch (e) { }
    return;
  }
...
async function openQishuiWebLogin() {
  ...
  if (!api || !api.isDesktop || typeof api.openQishuiMusicLogin !== 'function') {
    // 移动端: 打开 music.douyin.com 扫码登录页(应用内浏览器全屏),登录后自动获取Cookie并返回
    qishuiManualCookieOpen = true;
    updateLoginProviderUi();
    if (statusEl) { statusEl.innerHTML = '已打开汽水音乐登录页,请扫码登录,登录成功后将自动返回并同步。'; statusEl.className = 'preview'; }
    // 注册 Cookie 回调: 应用内浏览器检测到登录后自动回传 Cookie
    window._onQishuiLoginCookie = function (cookie) {
      if (!cookie) { if (statusEl) { statusEl.textContent = '未获取到 Cookie,请先在官网登录'; statusEl.className = 'fail'; } return; }
      if (statusEl) { statusEl.textContent = '已获取 Cookie,正在同步...'; statusEl.className = 'preview'; }
      // 自动提交 Cookie 到后端
      submitQishuiLoginCookie(cookie);
    };
    try {
      if (window.AndroidExternal && window.AndroidExternal.openQishuiMusicLogin) {
        window.AndroidExternal.openQishuiMusicLogin('https://music.douyin.com/');
      } else if (window.AndroidExternal && window.AndroidExternal.open) {
        window.AndroidExternal.open('https://music.douyin.com/');
      } else {
        window.open('https://music.douyin.com/', '_blank');
      }
    } catch (e) { }
    return;
  }
  ...
  updateLoginProviderUi();
  if (statusEl) { statusEl.textContent = '已打开汽水音乐窗口,请扫码并确认登录…'; statusEl.className = 'preview'; }
  try {
    var result = await api.openQishuiMusicLogin('https://music.douyin.com/');
    if (!result || !result.ok || !result.cookie) {
      throw new Error((result && (result.message || result.error)) || '汽水音乐登录未完成');
    }
    if (statusEl) { statusEl.textContent = '正在同步汽水音乐会话…'; statusEl.className = 'preview'; }
    await submitQishuiLoginCookie(result.cookie);
  } catch (e) {
    qishuiOAuthBusy = false;
    updateLoginProviderUi();
    if (statusEl) { statusEl.textContent = e && e.message ? e.message : '汽水音乐登录失败'; statusEl.className = 'fail'; }
  }
}

// 提交汽水音乐 Cookie 到后端进行登录验证
async function submitQishuiLoginCookie(cookie) {
  var statusEl = document.getElementById('qr-status');
  try {
    ...
    qishuiLoginStatus = Object.assign({}, qishuiLoginStatus, info);
    qishuiLoginStatus.loggedIn = true;
    activeAccountProvider = 'qishui';
    renderUserBtn();
    refreshUserPlaylists(true);
    var nickname = info.nickname || info.userId || '汽水用户';
    if (statusEl) { statusEl.textContent = '已登录汽水音乐 · ' + nickname; statusEl.className = 'scan'; }
    setTimeout(function () {
      closeLoginModal();
      showToast('汽水音乐已登录: ' + nickname);
    }, 420);
  } catch (e) {
    if (statusEl) { statusEl.textContent = e && e.message ? e.message : '汽水会话同步失败'; statusEl.className = 'fail'; }
    showToast('汽水登录失败: ' + (e && e.message ? e.message : '未知错误'));
  } finally {
    qishuiOAuthBusy = false;
    updateLoginProviderUi();
  }
