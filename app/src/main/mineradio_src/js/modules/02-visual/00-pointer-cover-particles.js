...
var _wheelRafPending = false;
var _wheelDeltaAccum = 0;
renderer.domElement.addEventListener('wheel', function (e) {
  ...
  requestAnimationFrame(function () {
    ...
    var delta = _wheelDeltaAccum;
    _wheelDeltaAccum = 0;
    if (freeCamera && freeCamera.active) {
      freeCamera.fov = clampRange((freeCamera.fov || BASE_FOV) + delta * 0.018, 26, 72);
      saveFreeCameraState();
      return;
    }
    if (fx && fx.preset === SKULL_PRESET_INDEX && typeof skullWheelZoomTarget !== 'undefined') {
      skullWheelZoomTarget = clampRange(skullWheelZoomTarget + delta * 0.00155, -0.95, 1.28);
      return;
    }
    idleGuideWheel({ deltaY: delta });
    unlockCenteredView();
    orbit.userRadius = Math.max(orbit.minRadius, Math.min(orbit.maxRadius, orbit.userRadius + delta * 0.005));
    if (orbit.recentering) orbit.recentering = false;
  });
}, { passive: false });
