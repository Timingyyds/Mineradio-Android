function normalizeCustomBackgroundImage(value) {
  ...
  // ★ 支持 content:// URI（Android FileProvider 本地资源）
  if (/^content:\/\//i.test(src)) return src;
  return '';
}
function normalizeCustomBackgroundMedia(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (/^data:video\/(mp4|webm|quicktime);base64,/i.test(value) || /^https?:\/\//i.test(value) || /^file:\/\//i.test(value)) return { type: 'video', src: String(value) };
    // ★ 支持 content:// URI（Android FileProvider 本地资源）
    if (/^content:\/\//i.test(value)) return { type: 'video', src: String(value) };
    return null;
  }
  ...
  if (type === 'video') {
    ...
    if (!id && !/^data:video\/(mp4|webm|quicktime);base64,/i.test(src) && !/^https?:\/\//i.test(src) && !/^content:\/\//i.test(src) && !/^file:\/\//i.test(src)) return null;
    return {
      type: 'video',
      id: id,
      src: src,
...
function applyColorLabValue(hex, silent) {
  ...
  // ★ Wallpaper Engine PMKG 场景参数颜色选择器（id 格式：we-prop-color-<propertyKey>）
  //   由 wallpaper-engine-library.js 渲染的 .we-prop-color.lyric-color-picker 触发
  //   通过 data-we-prop-key 属性获取场景参数 key，调原生接口实时应用
  else if (id && id.indexOf('we-prop-color-') === 0) {
    var weKey = colorLabState.picker && colorLabState.picker.getAttribute('data-we-prop-key');
    if (weKey && typeof wallpaperEngineApplyPropertyChange === 'function') {
      wallpaperEngineApplyPropertyChange(weKey, hex);
    }
  }
