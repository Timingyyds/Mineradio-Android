...
var lyricRenderUploadFrameBudget = { frame: 0, remaining: 3, consumed: 0, maxConsumed: 0 };
  ...
  for (var i = 0; i < data.rowLayers.length; i++) {
    ...
    if (row.mat && row.mat.uniforms) {
      if (row.mat.uniforms.uOpacity) {
        var lineOpacityTarget = data.usesTrack && ((initialTextRevealPending && !row.renderLineUploaded) || !lineLayerVisible || !row.renderLineUploaded) ? 0 : target * depthFade;
        if (initialTextRevealPending && !row.renderLineUploaded) row.mat.uniforms.uOpacity.value = 0;
        else {
      ...
      var fallbackLineOpacityTarget = data.usesTrack && ((initialTextRevealPending && !row.renderLineUploaded) || !lineLayerVisible || !row.renderLineUploaded) ? 0 : target * depthFade;
      if (initialTextRevealPending && !row.renderLineUploaded) row.mat.opacity = 0;
      else {
