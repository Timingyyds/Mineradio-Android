...
var stageLyricTrackGeneration = 0; var stageLyricPrewarmPendingSince = 0;
...
function stageLyricWarmupPending() {
  if (stageLyricPrewarm.mesh) { stageLyricPrewarmPendingSince = 0; return false; }
  if (stageLyricPrewarm.timer || stageLyricPrewarm.workTimer || stageLyricPrewarm.workRaf || stageLyricPrewarm.build) { if (!stageLyricPrewarmPendingSince) stageLyricPrewarmPendingSince = stageLyricNowMs(); if (stageLyricNowMs() - stageLyricPrewarmPendingSince > 1200) { stageLyricPrewarmPendingSince = 0; return false; } return true; }
...
function stageLyricCooperativeNextDelay(job, chunkMs) {
  ...
  if (chunkMs >= 8) return 12;
  if (chunkMs >= 5) return 8;
  if (chunkMs >= 3.5) return 4;
  return job && job.lightweight ? 0 : 3;
}
...
function runStageLyricCooperativePrewarm(job) {
  ...
  var done = false;
  try {
    done = stepCooperativeLyricMeshBuild(job.state, 4, 4.2);
  } catch (e) {
    cancelStageLyricPrewarmBuildOnly();
    return;
  }
...
function updateStageLyrics3D(dt) {
  ...
  function tickMesh(mesh, isCurrent) {
    ...
    if (isCurrent) {
      ...
      var opacityEase = shelfDetailOpen && currentOpacity > lyricOpacityTarget ? shelfDetailLyricProfile.easeDown : (currentOpacity <= 0.001 && lyricOpacityTarget > 0 ? 0.5 : 0.16);
