...
function notifyDesktopLyricsBeatMapReady() {
  try {
    ...
    if (androidDesktopLyricApi() && fx && fx.desktopLyrics) pushAndroidDesktopLyricState(true);
  } catch (e) { }
}
...
function applyDesktopLyricsState(force) {
  // ★ Android 原生悬浮窗（KeepApp 桥接）：Android WebView 中没有 desktopWindow，
  //   桌面窗口推送路径不可用，直接走 KeepApp 原生浮窗 + 50ms 同步定时器
  if (androidDesktopLyricApi()) {
    var isOn = !!(typeof fx !== 'undefined' && fx.desktopLyrics && !isDevelopmentLockedFx('desktopLyrics'));
    applyAndroidDesktopLyricState(isOn);
  }
  var api = getDesktopWindowApi();
  if (!api) return;
...
// ═══════════════════════════════════════════════════════════════════
//  ★ Android 原生桌面歌词桥接（KeepApp → DesktopLyricWindow，WebView 加载
//    与桌面版完全一致的 desktop-lyrics.html）
//  JS → Android：KeepApp.toggleDesktopLyric / updateDesktopLyricsState /
//                 updateDesktopLyricProgressFrac
//  Android → JS：window._onDesktopLyricPermissionGranted / _onDesktopLyricClosed /
//                 _onDesktopLyricLockState / _onDesktopLyricMoved
// ═══════════════════════════════════════════════════════════════════
var androidDesktopLyricSyncHandle = 0;
var androidDesktopLyricHiddenForStop = false;
var androidDesktopLyricStateAt = 0;
var androidDesktopLyricStateKey = '';
var androidDesktopLyricStateFontId = '';

function androidDesktopLyricApi() {
  return (typeof KeepApp !== 'undefined' && typeof KeepApp.toggleDesktopLyric === 'function') ? KeepApp : null;
}

function applyAndroidDesktopLyricState(isOn) {
  var api = androidDesktopLyricApi();
  if (!api) return;
  console.log('[DesktopLyric] android bridge toggle=' + !!isOn);
  try {
    api.toggleDesktopLyric(!!isOn);
    if (isOn) ensureAndroidDesktopLyricSync();
    else stopAndroidDesktopLyricSync();
  } catch (e) { console.warn('[DesktopLyric] Android state error:', e); }
}

/** 启动桌面歌词同步定时器（50ms）：推送完整状态 + 播放进度 */
function ensureAndroidDesktopLyricSync() {
  if (androidDesktopLyricSyncHandle) return; // 已启动
  // 立即推送一次完整状态与播放进度，让浮窗马上显示并开始动画
  pushAndroidDesktopLyricState(true);
  pushAndroidDesktopLyricProgress();
  androidDesktopLyricHiddenForStop = false;
  androidDesktopLyricSyncHandle = setInterval(function () {
    try {
      if (typeof KeepApp === 'undefined') return;
      // ★ 开关被关闭 → 停止定时器并隐藏
      if (typeof fx === 'undefined' || !fx || !fx.desktopLyrics) {
        try { if (typeof KeepApp.toggleDesktopLyric === 'function') KeepApp.toggleDesktopLyric(false); } catch (_) {}
        stopAndroidDesktopLyricSync();
        return;
      }
      // ★ 播放停止/暂停 → 隐藏浮窗（不改变 fx.desktopLyrics 开关）
      var isPlaying = false;
      try {
        isPlaying = typeof playing !== 'undefined' && playing && typeof audio !== 'undefined' && audio && !audio.paused && !audio.ended;
      } catch (_) {}
      if (!isPlaying) {
        if (!androidDesktopLyricHiddenForStop) {
          androidDesktopLyricHiddenForStop = true;
          try { if (typeof KeepApp.toggleDesktopLyric === 'function') KeepApp.toggleDesktopLyric(false); } catch (_) {}
        }
        return;
      }
      // ★ 恢复播放 → 重新显示并强制重新推送完整状态
      if (androidDesktopLyricHiddenForStop) {
        androidDesktopLyricHiddenForStop = false;
        try { if (typeof KeepApp.toggleDesktopLyric === 'function') KeepApp.toggleDesktopLyric(true); } catch (_) {}
        androidDesktopLyricStateKey = '';
        pushAndroidDesktopLyricState(true);
      }
      // 歌词/颜色/节拍变化时推送完整状态 + 高频推送播放进度
      pushAndroidDesktopLyricState(false);
      pushAndroidDesktopLyricProgress();
    } catch (_) {}
  }, 50);
}

function stopAndroidDesktopLyricSync() {
  if (androidDesktopLyricSyncHandle) {
    clearInterval(androidDesktopLyricSyncHandle);
    androidDesktopLyricSyncHandle = 0;
  }
}

/** 推送完整桌面歌词状态（与桌面版 desktopLyricsPayload 完全一致，限流 300ms） */
function pushAndroidDesktopLyricState(force) {
  var api = androidDesktopLyricApi();
  if (!api || typeof api.updateDesktopLyricsState !== 'function') return;
  var now = performance.now();
  if (!force && now - androidDesktopLyricStateAt < 300) return;
  var fontId = desktopLyricsCurrentCustomFontId();
  var includeFont = !!fontId && fontId !== androidDesktopLyricStateFontId;
  var payload = desktopLyricsPayload(false, includeFont);
  if (includeFont) androidDesktopLyricStateFontId = fontId;
  var colors = payload.colors || {};
  var motion = payload.motion || {};
  var key = payload.enabled + '|' + payload.text + '|' + Math.round(payload.progress * 1000) + '|' + payload.playing + '|' + payload.size + '|' + payload.opacity + '|' + payload.y + '|' + payload.clickThrough + '|' + payload.cinema + '|' + payload.highlightFollow + '|' + payload.beatMapKey + '|' + colors.primary + '|' + colors.glow + '|' + motion.lyricGlow;
  if (!force && key === androidDesktopLyricStateKey && now - androidDesktopLyricStateAt < 800) return;
  androidDesktopLyricStateAt = now;
  androidDesktopLyricStateKey = key;
  try {
    api.updateDesktopLyricsState(JSON.stringify(payload));
  } catch (e) { console.warn('[DesktopLyric] state push error:', e); }
}

/** 高频推送播放进度（0-1 比例 + 时长秒数，驱动逐字高亮与进度渐变） */
function pushAndroidDesktopLyricProgress() {
  try {
    if (typeof KeepApp !== 'undefined' && typeof KeepApp.updateDesktopLyricProgressFrac === 'function') {
      var t = 0;
      var dur = 0;
      try {
        if (typeof audio !== 'undefined' && audio) {
          t = audio.currentTime || 0;
          dur = isFinite(audio.duration) ? (audio.duration || 0) : 0;
        }
      } catch (_) {}
      var frac = dur > 0 ? Math.max(0, Math.min(1, t / dur)) : 0;
      KeepApp.updateDesktopLyricProgressFrac(frac, dur);
    }
  } catch (_) {}
}

// Android → JS：悬浮窗内用户操作回调（与桌面版 onDesktopLyrics* 语义一致）
window._onDesktopLyricClosed = function () {
  try {
    if (typeof fx === 'undefined' || fx.desktopLyrics === false) return;
    fx.desktopLyrics = false;
    updateFxInputs();
    saveLyricLayout({ user: true, reason: 'desktopLyrics' });
    stopAndroidDesktopLyricSync();
    if (typeof showToast === 'function') showToast('桌面歌词已关闭');
  } catch (_) {}
};
window._onDesktopLyricLockState = function (locked) {
  try {
    locked = !!locked;
    if (typeof fx === 'undefined' || fx.desktopLyricsClickThrough === locked) return;
    fx.desktopLyricsClickThrough = locked;
    updateFxInputs();
    saveLyricLayout({ user: true, reason: 'desktopLyricsClickThrough' });
    pushDesktopLyricsState(true);
    if (typeof showToast === 'function') showToast(locked ? '桌面歌词已锁定' : '桌面歌词可移动');
  } catch (_) {}
};
window._onDesktopLyricMoved = function (yFrac) {
  try {
    if (typeof fx === 'undefined') return;
    yFrac = clampRange(Number(yFrac), 0.08, 0.92);
    if (Math.abs(yFrac - fx.desktopLyricsY) < 0.0001) return;
    fx.desktopLyricsY = yFrac;
    saveLyricLayout({ user: true, reason: 'desktopLyricsY' });
  } catch (_) {}
};

// Android → JS：悬浮窗权限授予回调（native toggleDesktopLyric 返回后调用）
window._onDesktopLyricPermissionGranted = function (granted) {
  try {
    if (granted) {
      // 授权成功 → 启动歌词同步定时器 + 同步按钮状态
      ensureAndroidDesktopLyricSync();
      var btn = document.getElementById('t-desktopLyrics');
      if (btn) btn.classList.add('on');
    } else {
      // 授权被拒绝 → 取消按钮激活态 + 提示
      var btn2 = document.getElementById('t-desktopLyrics');
      if (btn2) btn2.classList.remove('on');
      try { if (typeof showToast === 'function') showToast('需要授予悬浮窗权限才能显示桌面歌词'); } catch (_) {}
    }
  } catch (_) {}
};
