...
var PLAYLIST_PANEL_OPEN_PROTECT_MS = 850;
var peekTimers = { search: null, fx: null, pl: null };
// 点击(而非鼠标悬停)打开歌单面板后的保护窗口:期间不因指针位置而强制关闭,
// 避免手机点击"秒开秒关"显得不灵敏。与 #fx-fab 的 fxFabOpenUntil 方案一致。
function markPlaylistPanelOpenProtected(ms) {
  window.__playlistPanelOpenUntil = performance.now() + (ms || PLAYLIST_PANEL_OPEN_PROTECT_MS);
}
function isPlaylistPanelOpenProtected() {
  return !!(window.__playlistPanelOpenUntil && performance.now() < window.__playlistPanelOpenUntil);
}
...
function setPeek(el, on, key) {
  ...
  // ★ DIY 关闭(simple-mode)时拒绝 fx 面板 peek，防止指针悬停误触发视觉控制台。
  //   on=false(关闭)始终放行。
  if (key === 'fx' && on && !diyPlayerMode) return;
...
var _peekPanelsRafPending = false;
var _peekPanelsLastEvent = null;
function handleGlobalPointerHover() {
  var e = _peekPanelsLastEvent;
  _peekPanelsRafPending = false;
  if (!e) return;
  ...
  if (immersiveMode) {
    ...
    else if (shouldClosePlaylistPanelFromPointer(ppOnImm, ex, ppRectImm, ey, H) && !isPlaylistPanelOpenProtected()) setPeek(pp, false, 'pl');
  ...
  // ★ 用户点击 #fx-fab 关闭后，短暂(hoverCloseUntil)内不因指针停留在按钮上而自动重新打开
  var fxHoverBlocked = !!(window.fxFabCloseUntil && performance.now() < window.fxFabCloseUntil);
  // ★ 点击打开后的保护窗口：此期间不因指针位置而强制关闭面板，避免手机点击"秒开秒关"显得不灵敏
  var fxOpenProtected = !!(window.fxFabOpenUntil && performance.now() < window.fxFabOpenUntil);
  if ((inFxFab || inFxPanel || inFxBridge) && !fxHoverBlocked) setPeek(fp, true, 'fx');
  else if (fpOn && !fxOpenProtected) setPeek(fp, false, 'fx');
  ...
  else if (shouldClosePlaylistPanelFromPointer(ppOn, ex, ppRect, ey, H) && !isPlaylistPanelOpenProtected()) setPeek(pp, false, 'pl');
...
window.addEventListener('mousemove', function (e) {
  _peekPanelsLastEvent = e;
  if (_peekPanelsRafPending) return;
  _peekPanelsRafPending = true;
  requestAnimationFrame(handleGlobalPointerHover);
});
