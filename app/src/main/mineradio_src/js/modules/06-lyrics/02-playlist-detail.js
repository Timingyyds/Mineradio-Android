...
// ★ 动态测量歌单详情行高（含行内歌手按钮幽灵间距），让虚拟滚动与实际渲染自校准。
//   修复：screen-rem-fitter 放大 html 字号后实际行高 ≠ PLAYLIST_DETAIL_ROW_STEP，
//   下滑累积偏移导致 20 多首后歌曲逐行"消失/显示不全"。
var __plDetailMeasuredStepCache = 0;
function playlistPanelDetailMeasuredRowStep() {
  var fallback = PLAYLIST_DETAIL_ROW_STEP;
  try {
    var detail = document.querySelector('.pl-inline-detail[data-pl-detail]');
    if (!detail) return fallback;
    var item = detail.querySelector('.pl-detail-row[data-pl-detail-row]');
    if (!item) return fallback;
    var rect = item.getBoundingClientRect();
    if (!rect || !rect.height || rect.height <= 0) return fallback;
    var step = 0;
    var next = item.nextElementSibling;
    if (next && next.classList && next.classList.contains('pl-detail-row')) {
      var nextRect = next.getBoundingClientRect();
      if (nextRect && nextRect.top > rect.top) {
        var d = nextRect.top - rect.top;
        if (d > rect.height * 0.5 && d < rect.height * 3) step = d;
      }
    }
    if (!step) step = rect.height;
    if (step > 4 && step < 240) __plDetailMeasuredStepCache = step;
    return __plDetailMeasuredStepCache || fallback;
...
// ★ 动态测量队列行高（含行间距），让虚拟滚动与实际渲染自校准。
//   修复：screen-rem-fitter 放大 html 字号或行高变化导致实际行高 ≠ QUEUE_VIRTUAL_ROW_STEP，
//   滚动时虚拟窗口位置与实际渲染位置累积偏移，歌曲在滚动中"消失"。
var __queueMeasuredStepCache = 0;
function queuePanelMeasuredRowStep() {
  var fallback = QUEUE_VIRTUAL_ROW_STEP;
  try {
    var list = document.getElementById('queue-list') || document.getElementById('mini-queue-list');
    if (!list) return fallback;
    var item = list.querySelector('.queue-item[data-queue-index],.mini-queue-item[data-queue-index]');
    if (!item) return fallback;
    var rect = item.getBoundingClientRect();
    if (!rect || !rect.height || rect.height <= 0) return fallback;
    // 用相邻兄弟行的顶部差作为"一行 + 间距"的真实步长，最准确
    var step = 0;
    var next = item.nextElementSibling;
    if (next && (next.classList.contains('queue-item') || next.classList.contains('mini-queue-item'))) {
      var nextRect = next.getBoundingClientRect();
      if (nextRect && nextRect.top > rect.top) {
        var d = nextRect.top - rect.top;
        if (d > rect.height * 0.5 && d < rect.height * 3) step = d; // 拒绝异常值
      }
    }
    if (!step) step = rect.height;
    // 缓存并夹取到合理范围，避免单次异常测量导致窗口塌缩（面板整片空白）
    if (step > 4 && step < 240) __queueMeasuredStepCache = step;
    return __queueMeasuredStepCache || fallback;
  } catch (e) { return fallback; }
}
function queuePanelVirtualWindow(list, scroller, total, selfScroll, forceIndex) {
  ...
  // ★ 队列数量在合理范围内时直接全量渲染，彻底避免虚拟窗口测量偏差导致歌曲"显示不全/消失"。
  //   只有超大队列才启用虚拟滚动（此时仍需保证窗口足够大）。
  if (total <= QUEUE_PANEL_FULL_RENDER_LIMIT) {
    return { start: 0, end: total, top: 0, bottom: 0 };
  }
  var rowStep = queuePanelMeasuredRowStep();
  ...
  // ★ 加大过扫描：在可视区域下方额外渲染足够多的行，即使快速下滑也不会露出空白。
  //   修复：下滑时"歌曲还没加载/显示不全"，让渲染窗口始终超前于滚动位置。
  var overscan = Math.max(QUEUE_VIRTUAL_OVERSCAN * 2, Math.ceil(visibleRows * 1.5));
  var start = 0;
  if (forceIndex != null && forceIndex >= 0 && forceIndex < total) {
    start = Math.max(0, Math.floor(forceIndex - visibleRows * 0.42) - overscan);
  } else if (selfScroll) {
    start = Math.max(0, Math.floor((Number(scroller && scroller.scrollTop) || 0) / rowStep) - overscan);
    ...
    start = Math.max(0, Math.floor(visibleTop / rowStep) - overscan);
  }
  var maxRows = visibleRows + overscan * 2;
...
function scheduleQueuePanelVirtualRender() {
  ...
  queuePanelVirtualState.raf = requestAnimationFrame(function () {
    ...
    // 队列在合理范围内时已全量渲染，无需在滚动时重建；只在虚拟滚动(超大队列)时才重建窗口
    var virtualized = (playQueue && playQueue.length || 0) > QUEUE_PANEL_FULL_RENDER_LIMIT;
    if (miniQueueOpen && virtualized) renderMiniQueuePanel({ animate: false, scrollCurrent: false });
    if (queueViewTab === 'queue' && isPlaylistPanelVisibleForRender() && virtualized) {
...
function maybeRequestPlaylistQueuePageFromScroller(scroller) {
  ...
  // ★ 提前很远就触发下一批歌曲加载（约 2 个视口高度），让数据始终超前于滚动位置，
  //   避免下滑时"加载出来的距离赶不上下滑距离"导致歌曲显示不全。
  var remaining = (Number(scroller.scrollHeight) || 0) - (Number(scroller.scrollTop) || 0) - (Number(scroller.clientHeight) || 0);
  var ahead = Math.max((Number(scroller.clientHeight) || 0) * 2, QUEUE_VIRTUAL_ROW_STEP * 40);
  if (remaining > ahead) return false;
...
function playlistPanelDetailRowsHtml(options) {
  ...
  var rowStep = playlistPanelDetailMeasuredRowStep();
  ...
  var start = Math.max(0, Math.floor(localScrollTop / rowStep) - PLAYLIST_DETAIL_VIRTUAL_OVERSCAN);
  var maxRows = Math.ceil(viewport / rowStep) + PLAYLIST_DETAIL_VIRTUAL_OVERSCAN * 2;
  ...
  var rows = '<div class="pl-detail-virtual-spacer" aria-hidden="true" style="height:' + (start * rowStep) + 'px"></div>';
  rows += tracks.slice(start, end).map(function (song, localIndex) {
    var i = start + localIndex;
    var thumb = songCoverSrc(song, 60);
    var imgTag = thumb ? '<img src="' + escHtml(thumb) + '" alt="" loading="lazy" decoding="async" onerror="this.style.opacity=0.2">' : '<div style="width:38px;height:38px;border-radius:6px;background:rgba(255,255,255,.06);flex:0 0 auto"></div>';
    return '<div class="pl-detail-row" data-pl-detail-row="' + i + '">' +
      ...
      '<div class="qi-act">' +
      '<button class="' + (isSongLiked(song) ? 'liked' : '') + '" onclick="event.stopPropagation();toggleLikeDetailSong(playlistPanelDetailState.tracks[' + i + '])" title="' + (isSongLiked(song) ? '取消红心' : '红心喜欢') + '">' + heartIconSvg() + '</button>' +
      '<button class="queue-next" onclick="event.stopPropagation();queueDetailSongNext(playlistPanelDetailState.tracks[' + i + '])" title="下一首播放">下</button>' +
      '<button onclick="event.stopPropagation();collectDetailSong(playlistPanelDetailState.tracks[' + i + '])" title="收藏到歌单">' + playlistPlusIconSvg() + '</button>' +
      '</div>' +
      '</div>';
  }).join('');
  rows += '<div class="pl-detail-virtual-spacer" aria-hidden="true" style="height:' + (Math.max(0, tracks.length - end) * rowStep) + 'px"></div>';
  if (st.error) {
...
function playlistPanelDetailShellHeight() {
  ...
  return PLAYLIST_DETAIL_OUTER_CHROME_HEIGHT + rows * rowStep + noticeHeight + footerHeight;
}
