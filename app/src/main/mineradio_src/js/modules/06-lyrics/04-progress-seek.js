...
var _progressThumbEl = null;
var _lastProgressPercent = -1;
var _lastTimeText = '';
function setProgressVisual(percent) {
  percent = clampRange(percent || 0, 0, 100);
  var q = Math.round(percent * 10) / 10;
  if (q === _lastProgressPercent) return;
  _lastProgressPercent = q;
  if (!_progressFillEl) _progressFillEl = document.getElementById('progress-fill');
  if (!_progressThumbEl) _progressThumbEl = document.getElementById('progress-thumb');
  if (_progressFillEl) _progressFillEl.style.width = q + '%';
  if (_progressThumbEl) _progressThumbEl.style.left = q + '%';
}
function updatePlaybackProgressUi() {
  ...
  if (timeText === _lastTimeText) return;
  _lastTimeText = timeText;
  var timeDisplay = document.getElementById('time-display');
  if (timeDisplay) timeDisplay.textContent = timeText;
  // ★ 同步播放进度到 Android 通知栏（节流：每 1.5 秒一次，避免高频 JS->Native 调用）
  if (typeof updatePlaybackProgressUi._lastNotifyMs === 'undefined') updatePlaybackProgressUi._lastNotifyMs = 0;
  var _nowMs = Date.now();
  if (audio && audio.readyState >= 1 && (_nowMs - updatePlaybackProgressUi._lastNotifyMs > 1500 || currentSec <= 1)) {
    updatePlaybackProgressUi._lastNotifyMs = _nowMs;
    try {
      if (typeof KeepApp !== 'undefined' && KeepApp && typeof KeepApp.updateMediaPosition === 'function') {
        KeepApp.updateMediaPosition(Math.round(currentSec * 1000), Math.round(durationSec * 1000));
      }
