...
var lyricFetchMissCache = {};
function lyricFetchMissKey(song) {
  return persistentLyricCacheKey(song);
}
function lyricFetchMissedRecently(song) {
  var key = lyricFetchMissKey(song);
  var missedAt = lyricFetchMissCache[key] || 0;
  return !!missedAt && Date.now() - missedAt < 10 * 60 * 1000;
}
function markLyricFetchMiss(song) {
  var key = lyricFetchMissKey(song);
  if (!key) return;
  lyricFetchMissCache[key] = Date.now();
}
function lyricResponseIsTemporaryFailure(response) {
  response = response || {};
  if (response.code === 503 || response.code === -1) return true;
  if (/service_unavailable|server not ready|timeout|econnreset|etimedout/i.test(String(response.message || response.error || ''))) return true;
  return false;
}
...
function shouldRetryLyricFetch(song, token, attempt) {
  ...
  if (lyricFetchMissedRecently(song)) return false;
  return true;
}
function scheduleLyricFetchRetry(song, token, attempt) {
  var delays = [900, 2600, 6500];
...
function resetLyricsForTrackSwitch() {
  cancelPendingTrackFallbackLyrics();
  // 切歌时立即用歌曲名称作为占位歌词渲染，避免旧歌词清空后、新歌词加载完成前出现"歌词消失"空窗。
  // 真歌词加载完成后 applyFetchedLyricResponse 会平滑替换掉占位歌词。
  var song = (typeof currentLyricSong === 'function') ? currentLyricSong() : null;
  var fallbackLines = withLyricFallbackForSong(song, []);
  ...
  lyricsLines = cloneLyricLines(fallbackLines);
...
async function fetchLyric(songOrId, token, attempt) {
  ...
  try {
    ...
    if (!state.usableLyric) {
      if (!lyricResponseIsTemporaryFailure(r)) markLyricFetchMiss(song);
      if (shouldRetryLyricFetch(song, token, attempt)) scheduleLyricFetchRetry(song, token, attempt);
    }
  } catch (e) {
...
// ─────────────── 本地 server 就绪后的歌词补偿获取 ───────────────
// 播放/切歌发生在 server 未就绪时, 歌词请求会拿到 503 并退回标题 fallback;
// server:ready 事件由 capacitor-bridge 广播, 在此重新拉取当前歌曲歌词。
window.addEventListener('mineradio-server-ready', function () {
  setTimeout(function () {
    try {
      var song = typeof currentLyricSong === 'function' ? currentLyricSong() : null;
      if (!song) return;
      if (hasUsableLyricLines(originalLyricsState && originalLyricsState.lines)) return;
      if (lyricFetchMissedRecently(song)) return;
      fetchLyric(song, trackSwitchToken, 0);
    } catch (e) { }
  }, 420);
});
