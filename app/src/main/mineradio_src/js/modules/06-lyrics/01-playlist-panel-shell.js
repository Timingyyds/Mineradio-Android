...
function togglePlaylistPanel(force) {
  ...
  if (force === false) el.classList.remove('show');
    ...
    if (typeof markPlaylistPanelOpenProtected === 'function') markPlaylistPanelOpenProtected();
...
function queueRowsRenderKey(songs, start, end) {
  var out = '';
  for (var i = start; i < end; i++) out += queueItemKey(songs[i]) + '\u0001';
  return out;
}
function updateQueuePanelRowsInPlace($ql, songs, nowIdx) {
  var rows = $ql.children;
  for (var k = 0; k < rows.length; k++) {
    var row = rows[k];
    if (!row.classList || !row.classList.contains('queue-item')) continue;
    var idx = Number(row.getAttribute('data-queue-index'));
    if (!isFinite(idx)) continue;
    var song = songs[idx];
    row.classList.toggle('now', idx === nowIdx);
    if (!song) continue;
    var qiAct = row.querySelector('.qi-act');
    var likeBtn = qiAct && qiAct.querySelector('button');
    if (likeBtn) {
      var liked = isSongLiked(song);
      likeBtn.classList.toggle('liked', liked);
      likeBtn.title = liked ? '取消红心' : '红心喜欢';
    }
  }
}
function updateMiniQueueRowsInPlace($list, nowIdx) {
  var rows = $list.children;
  for (var k = 0; k < rows.length; k++) {
    var row = rows[k];
    if (!row.classList || !row.classList.contains('mini-queue-item')) continue;
    var idx = Number(row.getAttribute('data-queue-index'));
    if (!isFinite(idx)) continue;
    row.classList.toggle('now', idx === nowIdx);
  }
...
function renderMiniQueuePanel(opts) {
  ...
  if (!total) {
    ...
    delete $list.dataset.miniQueueRenderKey;
    return;
  }
  var windowInfo = queuePanelVirtualWindow($list, $list, total, true, opts.scrollCurrent ? currentIdx : -1);
  var renderKey = total + ':' + windowInfo.start + ':' + windowInfo.end + ':' + queueRowsRenderKey(playQueue, windowInfo.start, windowInfo.end);
  if ($list.dataset.miniQueueRenderKey === renderKey && $list.children.length) {
    updateMiniQueueRowsInPlace($list, currentIdx);
  ...
  $list.dataset.miniQueueRenderKey = renderKey;
...
function renderQueuePanel(opts) {
  ...
  if (!playQueue.length) {
    ...
    delete $ql.dataset.queueRenderKey;
  ...
  var renderKey = total + ':' + windowInfo.start + ':' + windowInfo.end + ':' + queueRowsRenderKey(playQueue, windowInfo.start, windowInfo.end);
  if ($ql.dataset.queueRenderKey === renderKey && $ql.children.length) {
    updateQueuePanelRowsInPlace($ql, playQueue, currentIdx);
    renderMiniQueuePanel({ scrollCurrent: opts.scrollCurrent !== false && miniQueueOpen });
    return;
  }
  $ql.dataset.queueRenderKey = renderKey;
  var visibleQueue = playQueue.slice(windowInfo.start, windowInfo.end);
