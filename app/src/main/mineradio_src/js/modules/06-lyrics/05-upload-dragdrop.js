...
async function restorePersistedLocalLibrary() {
  console.log('[AutoplayDiag] restorePersistedLocalLibrary: desktopWindow=' + !!(window.desktopWindow && typeof window.desktopWindow.listLocalMusicLibrary === 'function') + ' restoredSnapshot=' + !!restoredLastPlaybackSnapshot + ' playQueue=' + playQueue.length + ' idx=' + currentIdx);
  ...
  console.log('[AutoplayDiag] restorePersistedLocalLibrary: result ok=' + (result && result.ok) + ' tracks=' + (result && Array.isArray(result.tracks) ? result.tracks.length : '-'));
  ...
  if (snapshot && !isLocalPlaybackSnapshot(snapshot)) { console.log('[AutoplayDiag] restorePersistedLocalLibrary: snapshot not local, skip'); return false; }
  if (
  ...
  if (snapshot) {
    ...
    console.log('[AutoplayDiag] restorePersistedLocalLibrary: restoredIndex=' + restoredIndex);
    if (restoredIndex < 0 || !tracks[restoredIndex]) {
      console.log('[AutoplayDiag] restorePersistedLocalLibrary: local track MISSING -> playQueue=' + tracks.length + ' idx=-1 snapshot cleared');
      playQueue = tracks;
      currentIdx = -1;
    ...
    console.log('[AutoplayDiag] restorePersistedLocalLibrary: restored idx=' + restoredIndex + ' playQueue=' + tracks.length);
  } else {
    currentIdx = -1;
  }
  playQueue = tracks;
