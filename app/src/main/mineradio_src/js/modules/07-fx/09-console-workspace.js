...
var FX_CONSOLE_LAYOUT = [
  {
    ...
    groups: [
      ...
      { key: 'reset', title: '恢复与整理', hint: '恢复全部默认参数', items: [
        fxConsoleItem({ selector: '#fx-reset-action' }, '恢复默认', '重置 全部默认')
      ] }
    ]
  },
  {
  ...
  },
  {
    ...
    groups: [
      ...
      { key: 'plugins', title: '插件管理', hint: '浏览、下载、安装和管理插件', items: [
        fxConsoleItem({ selector: '#plugin-install-action' }, '安装插件', '插件 安装 本地 zip mr 离线', false),
        fxConsoleItem({ selector: '#plugin-manage-action' }, '管理插件', '插件 管理 启用 禁用 卸载', false)
      ] },
...
function fxConsoleMakeGroup(page, tabMeta, groupMeta) {
  ...
  // ★ 强制展开所有分组，确保每一个按钮都显示出来（面板已加高并缩至80%，可滚动查看）
  fold.className = 'fx-fold fx-console-group open';
