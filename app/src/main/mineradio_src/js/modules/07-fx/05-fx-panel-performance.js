var homeWaveTrackState = { bars: 0, smooth: [], heights: [], opacities: [] };
...
function updateHomeAudioVisual(dt) {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    var hPx = Math.round(Math.max(4, prev * 18));
    var oVal = Math.round(clampRange(0.36 + prev * 0.68, 0.32, 1) * 20) / 20;
    if (hPx !== homeWaveTrackState.heights[i]) {
      homeWaveTrackState.heights[i] = hPx;
      bars[i].style.height = hPx + 'px';
    }
    if (oVal !== homeWaveTrackState.opacities[i]) {
      homeWaveTrackState.opacities[i] = oVal;
      bars[i].style.opacity = String(oVal);
    }
  }
}
...
function setPerformanceQualityMode(mode, silent) {
  ...
  applyRendererPowerMode();
  // ★ 画质档位 → 歌词文字抗锯齿档位 联动
  //   文字越小锯齿越明显，画质档位越高则歌词纹理超采样倍数越高（抗锯齿越强）
  //   低=1×、中=2×、高=3×、超高=4×
  var clarityForQuality = { eco: 1, balanced: 2, high: 3, ultra: 4 }[next] || 1;
  if (typeof setLyricTextureClarity === 'function') {
    setLyricTextureClarity(clarityForQuality, true);
  } else {
    // 兜底：直接更新 fx 并让调用方按需刷新
    fx.lyricTextureClarity = clarityForQuality;
  }
  saveLyricLayout({ user: true, reason: 'performanceQuality' });
  if (!silent) {
    var label = next === 'eco' ? '低' : (next === 'balanced' ? '中' : (next === 'ultra' ? '超高' : '高'));
    showToast('画质档位: ' + label + '（歌词抗锯齿 ' + (next === 'eco' ? '1×' : (next === 'balanced' ? '2×' : (next === 'ultra' ? '4×' : '3×'))) + '）');
  }
}
