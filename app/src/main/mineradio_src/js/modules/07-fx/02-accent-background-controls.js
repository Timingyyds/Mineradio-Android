...
function setCustomBackgroundMedia(media, silent) {
  ...
  fx.backgroundAlbumCover = false;
  if (!media && window.KeepApp && typeof window.KeepApp.wallpaperClearAppBg === 'function') {
    try { window.KeepApp.wallpaperClearAppBg(); } catch (e) { console.warn('wallpaperClearAppBg failed:', e); }
  }
  updateCustomBackgroundControls();
...
function readBackgroundImageFile(file) {
  // ★ 修复：Android 文件管理器选中的文件 file.type 常为空，需按扩展名补充判断
  if (!file || !(/^image\//i.test(file.type || '') || /\.(png|jpe?g|gif|webp|bmp|svg|avif)$/i.test(file.name || ''))) {
    showToast('请选择图片文件');
    return;
  }
...
function readBackgroundVideoFile(file) {
  // ★ 修复：Android 文件管理器选中的文件 file.type 常为空，需按扩展名补充判断
  if (!file || !(/^video\//i.test(file.type || '') || /\.(mp4|webm|ogv|mov|m4v|mkv|avi)$/i.test(file.name || ''))) {
    showToast('请选择视频文件');
    return;
  }
