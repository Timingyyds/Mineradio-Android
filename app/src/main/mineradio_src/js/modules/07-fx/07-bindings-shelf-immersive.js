function bindFxPanel() {
  ...
  ids.forEach(function (pair) {
    ...
    el.addEventListener('input', function () {
      ...
      if (pair[1] === 'desktopLyricsSize') fx.desktopLyricsSize = clampRange(fx.desktopLyricsSize, 0.10, 1.55);
...
function toggleFxPanel(force) {
  ...
  if (!el) return;
  // ★ DIY 关闭(simple-mode)时拒绝打开视觉控制台，防止按钮误触发面板弹出。
  //   force===false(关闭)始终放行，确保能正常关闭。
  if (!diyPlayerMode && force !== false) {
    if (typeof showToast === 'function') showToast('开启 DIY 玩家模式后可打开视觉控制台');
    return;
  }
  // ★ 仅把 show(用户点击固定打开)视为"已打开"；peek 只是悬停/触摸预览，
  //    点击时应将其升级为 show 而不是误判为关闭，避免手机端"一点就秒开秒关"
  var currentlyOpen = el.classList.contains('show');
  if (peekTimers && peekTimers.fx) { clearTimeout(peekTimers.fx); peekTimers.fx = null; }
  fxPanelPinned = false;
  if (force === false) {
    el.classList.remove('show', 'peek');
    el.classList.toggle('closing', currentlyOpen || el.classList.contains('peek'));
    setTimeout(function () { el.classList.remove('closing'); }, 280);
    var fab = document.getElementById('fx-fab');
    if (fab) fab.classList.remove('active');
    return;
  }
  // ★ 点击 #fx-fab：若视觉控制台已固定打开则关闭，否则打开(开关切换)
  if (currentlyOpen) {
    el.classList.remove('show', 'peek');
    el.classList.toggle('closing', true);
    setTimeout(function () { el.classList.remove('closing'); }, 280);
    var fab = document.getElementById('fx-fab');
    if (fab) fab.classList.remove('active');
    // ★ 关闭后短暂拦截 hover 自动重新打开，确保点击关闭真正生效
    window.fxFabCloseUntil = performance.now() + 500;
    return;
  }
  el.classList.remove('show', 'closing');
  setPeek(el, true, 'fx');
  // ★ 点击打开后固定显示(show)，避免被 pointermove/hover 逻辑秒关，保证“点击一下即打开”
  el.classList.add('show');
  window.fxPanelPinned = true;
  // ★ 打开后设定保护窗口，防止紧接着的 pointermove/touchmove 又触发关闭
  window.fxFabOpenUntil = performance.now() + 700;
}
function resetFx() {
  var savedCam = fx.cam;
...
function setImmersiveMode(on) {
  ...
  if (on) {
    ...
    setTimeout(function () {
      if (immersiveMode) {
        // 触摸设备上 hover 无意义，强制隐藏控制条，避免沉浸模式下控制条残留不自动隐藏
        if (IS_TOUCH_DEVICE) controlsHovering = false;
