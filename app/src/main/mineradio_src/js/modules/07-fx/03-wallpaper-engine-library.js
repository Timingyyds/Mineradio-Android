...
function wallpaperEngineMediaUrl(item, kind) {
  ...
  // ★ 本地导入的图片/视频（非 PMKG Scene）：Android WebView 不拦截
  //   mineradio-wallpaper:// 协议，直接用 blob URL，否则切换/显示会失败
  if (item.source === 'local' && item.projectType !== 'scene') {
    var localEntry = wallpaperEngineLocalMediaMap.get(id);
    if (localEntry && localEntry.url) return localEntry.url;
  }
  return 'mineradio-wallpaper://' + kind + '/' + encodeURIComponent(id) + '?v=' + encodeURIComponent(String(item.updatedAt || 0)) + '&token=' + encodeURIComponent(wallpaperEngineMediaToken);
}
...
function activateWallpaperEngineItem(id) {
  ...
  // ★ PMKG 场景壁纸：通过原生层启动预览（Android WebView 无法直接渲染 PMKG 场景）
  if (item.source === 'local' && item.projectType === 'scene' && item.safetyMode === 'native-engine') {
    var localEntry = wallpaperEngineLocalMediaMap.get(id);
    var pmkgName = (localEntry && localEntry.name) || item.title || '';
    try {
      if (window.KeepApp && typeof window.KeepApp.launchMpkgPreview === 'function') {
        // ★ 记录选中状态，使恢复按钮可用并在重启后重新应用
        ...
        document.body.classList.add('wallpaper-engine-active');
        updateWallpaperEngineEntryUi(item.title + ' · 已应用');
        showToast('正在应用到程序背景: ' + (item.title || pmkgName));
        window.KeepApp.launchMpkgPreview(pmkgName);
        closeWallpaperEngineLibrary();
        return;
      }
    } catch (e) { console.warn('[WE] launchMpkgPreview failed: ' + e); }
    showToast('PMKG 场景需要原生支持');
    return;
  }
  wallpaperEngineSelection = normalizeWallpaperEngineSelection({
    active: true,
...
function deactivateWallpaperEngineBackground(quiet) {
  ...
  // ★ 无条件清除应用背景持久化（无论壁纸来源/类型，避免重启后自动恢复旧壁纸导致黑屏）
  try {
    if (window.KeepApp && typeof window.KeepApp.wallpaperClearAppBg === 'function') window.KeepApp.wallpaperClearAppBg();
  } catch (e) { console.warn('[WE] wallpaperClearAppBg failed: ' + e); }
  // ★ 同步清除系统壁纸记录并关闭强制系统壁纸开关，避免重启后保活机制再次激活旧壁纸导致黑屏
  try {
    if (window.KeepApp && typeof window.KeepApp.wallpaperClearSystem === 'function') window.KeepApp.wallpaperClearSystem();
    if (window.KeepApp && typeof window.KeepApp.setForceSystemWallpaper === 'function') window.KeepApp.setForceSystemWallpaper('0');
  } catch (e) { console.warn('[WE] clear system wallpaper failed: ' + e); }
  if (wallpaperEngineHostBoundsRestartTimer) {
    clearTimeout(wallpaperEngineHostBoundsRestartTimer);
    wallpaperEngineHostBoundsRestartTimer = 0;
  }
...
function renderWallpaperEngineLibrary(preserveRenderLimit) {
  ...
  grid.innerHTML = visibleItems.map(function (item) {
    ...
    // ★ 本地 PMKG 项目优先使用原生提取的封面 data URL（item.previewData），否则用协议 URL
    var preview = item.previewData || (item.hasPreview ? wallpaperEngineMediaUrl(item, 'preview') : '');
...
// ★ PMKG 文本净化：解码 HTML 实体/URL 编码/Unicode 转义，美化 i18n key
//   解决 PMKG 原生 text/options.label 在 i18n 未匹配时返回原 key（如 "Property.Options.Color"）
//   或包含 HTML 实体（&amp;）、URL 编码（%E4%B8%AD）、Unicode 转义（\u4e2d\u6587）等"乱码"
function wallpaperEnginePmkgLabelText(text, fallback) {
  ...
  // 解码 URL 编码（%E4%B8%AD%E6%96%87 → 中文）
  if (/%[0-9a-fA-F]{2}/.test(text)) {
    try { text = decodeURIComponent(text); } catch (e) {}
  }
  // 解码 HTML 实体（&amp; &#x4e2d; &#20013; &lt;）— 用 DOM 解码最稳妥
  if (/[<>&]/.test(text)) {
    try {
  ...
  // 控制字符清理
  text = text.replace(/[\u0000-\u001f\u007f]/g, ' ');
  // 美化 i18n key：纯 ASCII 含 . 或 _ 时替换为空格
  //   "Property.Options.Color" → "Property Options Color"
  //   "label_water_level" → "label water level"
  //   "ui_color_red" → "ui color red"
  if (/^[A-Za-z0-9][A-Za-z0-9_.\- ]*$/.test(text) && /[._]/.test(text)) {
...
function normalizeWallpaperEngineProjectDetails(details) {
  ...
  var properties = Array.isArray(details.properties) ? details.properties.slice(0, 256).map(function (property) {
    ...
    var options = Array.isArray(property.options) ? property.options.slice(0, 64).map(function (option) {
      ...
      return {
        label: wallpaperEnginePmkgLabelText(option.label, '选项').slice(0, 160),
        value: optionValue
      };
    }).filter(Boolean) : [];
    // ★ PMKG 原生字段保留：min/max/step/precision（用于滑块渲染）
    var min = typeof property.min === 'number' ? property.min : null;
    var max = typeof property.max === 'number' ? property.max : null;
    var step = typeof property.step === 'number' ? property.step : null;
    var precision = typeof property.precision === 'number' ? property.precision : null;
    return {
      key: key,
      label: wallpaperEnginePmkgLabelText(property.label, key).slice(0, 160) || key,
...
function renderWallpaperEngineProjectDetails(details, error) {
  ...
  var resetButton = document.getElementById('wallpaper-engine-details-reset');
  if (!drawer || !title || !summary || !properties) return;
  drawer.classList.add('show');
  drawer.setAttribute('aria-hidden', 'false');
  // ★ 重置按钮仅在 PMKG 可编辑模式显示
  if (resetButton) resetButton.style.display = (details && details.editable) ? '' : 'none';
  if (error) {
    title.textContent = '项目设置';
    summary.textContent = '';
    properties.innerHTML = '<div class="wallpaper-engine-details-empty">' + escHtml(error) + '</div>';
  ...
  summary.textContent = details.editable
    ? '已加载 ' + details.propertyCount + ' 项可调参数 · 调整后立即生效并持久化保存'
    : '已读取 ' + details.propertyCount + ' 项设置 · 检测到 ' + details.audioPropertyCount +
  ...
  properties.innerHTML = details.properties.length ? details.properties.map(function (property) {
    var badge = property.audio
      ...
      : '';
    var control = details.editable
      ? '<div class="wallpaper-engine-property-control">' + wallpaperEnginePropertyControl(property) + '</div>'
      : '<span class="wallpaper-engine-property-value">' + escHtml(wallpaperEnginePropertyValueLabel(property)) + '</span>';
    return '<div class="wallpaper-engine-property-row' + (details.editable ? ' editable' : '') + '" data-property-key="' + escHtml(property.key) + '">' +
  ...
  // ★ 可编辑模式：绑定控件事件，调 applyMpkgSceneProperty 实时生效
  if (details.editable) bindWallpaperEnginePropertyControls(properties);
...
// ★ 根据 property.type 渲染 PMKG 可编辑控件（开关/滑块/选项按钮组/输入框/颜色）
function wallpaperEnginePropertyControl(property) {
  var type = String(property.type || '').toLowerCase();
  var value = property.value;
  // bool → 开关
  if (type === 'bool' || type === 'boolean') {
    var checked = value === true || value === 'true' || value === 1 || value === '1' ? ' checked' : '';
    return '<label class="we-prop-toggle"><input type="checkbox" data-we-prop-input="bool"' + checked + '><span class="we-prop-toggle-track"></span></label>';
  }
  // combo / select / 有 options → 选项按钮组
  if ((type === 'combo' || type === 'select' || type === 'options') && property.options && property.options.length) {
    return '<div class="we-prop-options" data-we-prop-input="options">' + property.options.map(function (opt) {
      var active = String(opt.value) === String(value) ? ' active' : '';
      return '<button type="button" class="fx-mini-btn we-prop-option' + active + '" data-we-prop-value="' + escHtml(String(opt.value)) + '">' + escHtml(opt.label) + '</button>';
    }).join('') + '</div>';
  }
  // slider / number with min/max → 滑块
  if ((type === 'slider' || type === 'number') && typeof property.min === 'number' && typeof property.max === 'number') {
    var min = property.min;
    var max = property.max;
    var step = typeof property.step === 'number' && property.step > 0 ? property.step : (max - min) / 100;
    var numVal = typeof value === 'number' ? value : (parseFloat(value) || min);
    if (numVal < min) numVal = min;
    if (numVal > max) numVal = max;
    var precision = typeof property.precision === 'number' && property.precision >= 0 ? property.precision : 2;
    return '<div class="we-prop-slider-row">' +
      '<input type="range" class="we-prop-slider" data-we-prop-input="slider" min="' + min + '" max="' + max + '" step="' + step + '" value="' + numVal + '" data-precision="' + precision + '">' +
      '<span class="we-prop-slider-value">' + numVal.toFixed(precision) + '</span></div>';
  }
  // textinput → 输入框（text/string 是只读展示，不当输入框处理）
  if (type === 'textinput') {
    var strVal = typeof value === 'string' ? value : String(value == null ? '' : value);
    return '<input type="text" class="we-prop-text" data-we-prop-input="text" value="' + escHtml(strVal) + '">';
  }
  // color → 颜色选择（自定义颜色按钮，点击弹出独立颜色面板）
  if (type === 'color' && typeof value === 'string') {
    var hex = /^#?[0-9a-f]{6}$/i.test(value) ? (value.charAt(0) === '#' ? value : '#' + value) : '#000000';
    var colorKey = String(property.key || '');
    var colorBtnId = 'we-prop-color-' + colorKey.replace(/[^a-zA-Z0-9_-]/g, '_');
    return '<button type="button" id="' + escHtml(colorBtnId) + '" class="we-prop-color-btn" data-we-prop-input="color" data-we-prop-key="' + escHtml(colorKey) + '" data-we-prop-color="' + hex + '" style="--we-color:' + hex + '" title="点击选择颜色"><span class="we-prop-color-swatch"></span></button>';
  }
  // 兜底：只读展示
  return '<span class="wallpaper-engine-property-value">' + escHtml(wallpaperEnginePropertyValueLabel(property)) + '</span>';
}

// ★ 绑定编辑控件事件 → 调 window.KeepApp.applyMpkgSceneProperty(key, value)
function bindWallpaperEnginePropertyControls(container) {
  if (!container) return;
  container.querySelectorAll('input[data-we-prop-input="bool"]').forEach(function (input) {
    input.addEventListener('change', function () {
      var row = input.closest('.wallpaper-engine-property-row');
      var key = row && row.getAttribute('data-property-key');
      if (key) wallpaperEngineApplyPropertyChange(key, input.checked ? 'true' : 'false');
    });
  });
  container.querySelectorAll('[data-we-prop-input="options"]').forEach(function (group) {
    group.querySelectorAll('.we-prop-option').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var row = btn.closest('.wallpaper-engine-property-row');
        var key = row && row.getAttribute('data-property-key');
        if (!key) return;
        var value = btn.getAttribute('data-we-prop-value');
        group.querySelectorAll('.we-prop-option').forEach(function (b) { b.classList.remove('active'); });
        btn.classList.add('active');
        wallpaperEngineApplyPropertyChange(key, value);
      });
    });
  });
  container.querySelectorAll('input[data-we-prop-input="slider"]').forEach(function (input) {
    var valueLabel = input.parentNode.querySelector('.we-prop-slider-value');
    var precision = parseInt(input.getAttribute('data-precision') || '2', 10);
    if (isNaN(precision)) precision = 2;
    input.addEventListener('input', function () {
      if (valueLabel) valueLabel.textContent = parseFloat(input.value).toFixed(precision);
    });
    input.addEventListener('change', function () {
      var row = input.closest('.wallpaper-engine-property-row');
      var key = row && row.getAttribute('data-property-key');
      if (key) wallpaperEngineApplyPropertyChange(key, input.value);
    });
  });
  container.querySelectorAll('input[data-we-prop-input="text"]').forEach(function (input) {
    input.addEventListener('change', function () {
      var row = input.closest('.wallpaper-engine-property-row');
      var key = row && row.getAttribute('data-property-key');
      if (key) wallpaperEngineApplyPropertyChange(key, input.value);
  ...
  container.querySelectorAll('button[data-we-prop-input="color"]').forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      var key = btn.getAttribute('data-we-prop-key') || '';
      var color = btn.getAttribute('data-we-prop-color') || '#000000';
      openWallpaperEngineColorPanel(btn, key, color, function (newColor) {
        // 更新按钮显示色
        btn.style.setProperty('--we-color', newColor);
        btn.setAttribute('data-we-prop-color', newColor);
        // 实时应用到场景
        wallpaperEngineApplyPropertyChange(key, newColor);
      });
    });
  });
}

// ★ 自定义颜色选择面板（独立实现，不依赖 color-lab）
//   包含：色相条 + SV 选择区 + hex 输入 + 预设色
var __weColorPanel = null;
var __weColorState = { h: 0, s: 1, v: 1, key: '', callback: null, anchor: null };

function weHexToHsv(hex) {
  hex = hex.replace('#', '');
...
function weHsvToHex(h, s, v) {
  ...
  var toHex = function (n) { var s = Math.round(n * 255).toString(16); return s.length < 2 ? '0' + s : s; };
  return '#' + toHex(r) + toHex(g) + toHex(b);
}

function weHsvToRgbHex(h, s, v) {
  return weHsvToHex(h, s, v);
}

function weSyncColorPanel(hex) {
  var hsv = weHexToHsv(hex);
  __weColorState.h = hsv.h; __weColorState.s = hsv.s; __weColorState.v = hsv.v;
  var panel = __weColorPanel;
  if (!panel) return;
  var sv = panel.querySelector('.we-cp-sv');
  var cursor = panel.querySelector('.we-cp-cursor');
  var hueSlider = panel.querySelector('.we-cp-hue');
  var hexInput = panel.querySelector('.we-cp-hex');
  var preview = panel.querySelector('.we-cp-preview');
  var hueHex = weHsvToHex(hsv.h, 1, 1);
  if (sv) sv.style.background = 'linear-gradient(to top, #000, transparent), linear-gradient(to right, #fff, ' + hueHex + ')';
  if (cursor) { cursor.style.left = (hsv.s * 100) + '%'; cursor.style.top = ((1 - hsv.v) * 100) + '%'; }
  if (hueSlider) hueSlider.value = Math.round(hsv.h * 360);
  if (hexInput) hexInput.value = hex.toUpperCase();
  if (preview) preview.style.background = hex;
}

function weApplyColorFromPanel() {
  var hex = weHsvToHex(__weColorState.h, __weColorState.s, __weColorState.v);
  var hexInput = __weColorPanel && __weColorPanel.querySelector('.we-cp-hex');
  if (hexInput) hex = hexInput.value || hex;
  hex = /^#?[0-9a-f]{6}$/i.test(hex) ? (hex.charAt(0) === '#' ? hex : '#' + hex) : '#000000';
  if (__weColorState.callback) __weColorState.callback(hex);
  return hex;
}

function closeWallpaperEngineColorPanel() {
  if (__weColorPanel) {
    __weColorPanel.classList.remove('show');
  }
  __weColorState.callback = null;
  __weColorState.anchor = null;
}

function openWallpaperEngineColorPanel(anchor, key, color, callback) {
  if (!__weColorPanel) {
    // 创建面板 DOM（只创建一次）
    __weColorPanel = document.createElement('div');
    __weColorPanel.className = 'we-color-panel';
    __weColorPanel.innerHTML =
      '<div class="we-cp-head"><span class="we-cp-title">颜色</span><button type="button" class="we-cp-close">×</button></div>' +
      '<div class="we-cp-sv"><div class="we-cp-cursor"></div></div>' +
      '<div class="we-cp-row">' +
        '<div class="we-cp-preview"></div>' +
        '<input type="range" class="we-cp-hue" min="0" max="360" step="1" value="0">' +
        '<input type="text" class="we-cp-hex" maxlength="7" value="#000000">' +
      '</div>' +
      '<div class="we-cp-presets"></div>';
    document.body.appendChild(__weColorPanel);

    // 关闭按钮
    __weColorPanel.querySelector('.we-cp-close').addEventListener('click', closeWallpaperEngineColorPanel);

    // SV 选择区拖拽
    var sv = __weColorPanel.querySelector('.we-cp-sv');
    var updateFromSv = function (e) {
      ...
      __weColorState.s = x;
      __weColorState.v = 1 - y;
      var hex = weHsvToHex(__weColorState.h, __weColorState.s, __weColorState.v);
      weSyncColorPanel(hex);
      if (__weColorState.callback) __weColorState.callback(hex);
    };
    var svDragging = false;
    sv.addEventListener('pointerdown', function (e) { e.preventDefault(); svDragging = true; updateFromSv(e); });
    document.addEventListener('pointermove', function (e) { if (svDragging) updateFromSv(e); });
    document.addEventListener('pointerup', function () { svDragging = false; });

    // 色相滑块
    var hueSlider = __weColorPanel.querySelector('.we-cp-hue');
    hueSlider.addEventListener('input', function () {
      __weColorState.h = (parseFloat(hueSlider.value) || 0) / 360;
      var hex = weHsvToHex(__weColorState.h, __weColorState.s, __weColorState.v);
      weSyncColorPanel(hex);
      if (__weColorState.callback) __weColorState.callback(hex);
    });
    ...
    // hex 输入
    var hexInput = __weColorPanel.querySelector('.we-cp-hex');
    hexInput.addEventListener('change', function () {
      var v = hexInput.value.trim();
      var hex = /^#?[0-9a-f]{6}$/i.test(v) ? (v.charAt(0) === '#' ? v : '#' + v) : '#000000';
      weSyncColorPanel(hex);
      if (__weColorState.callback) __weColorState.callback(hex);
    });

    // 预设颜色
    var presets = ['#FFFFFF', '#000000', '#FF3B30', '#FF9500', '#FFCC00', '#34C759', '#00C7BE', '#30B0C7', '#007AFF', '#5856D6', '#AF52DE', '#FF2D55'];
    var presetEl = __weColorPanel.querySelector('.we-cp-presets');
    presetEl.innerHTML = presets.map(function (c) {
      return '<button type="button" class="we-cp-preset" style="background:' + c + '" data-color="' + c + '"></button>';
    }).join('');
    presetEl.querySelectorAll('.we-cp-preset').forEach(function (p) {
      p.addEventListener('click', function () {
        var hex = p.getAttribute('data-color');
        weSyncColorPanel(hex);
        if (__weColorState.callback) __weColorState.callback(hex);
      });
    });
    ...
    document.addEventListener('pointerdown', function (e) {
      if (!__weColorPanel || !__weColorPanel.classList.contains('show')) return;
      if (e.target === __weColorPanel || __weColorPanel.contains(e.target)) return;
      if (__weColorState.anchor && (__weColorState.anchor === e.target || __weColorState.anchor.contains(e.target))) return;
      closeWallpaperEngineColorPanel();
    });
  }

  __weColorState.key = key;
  __weColorState.callback = callback;
  __weColorState.anchor = anchor;
  weSyncColorPanel(color);
  __weColorPanel.classList.add('show');

  // 定位面板到按钮附近
  if (anchor && anchor.getBoundingClientRect) {
    var rect = anchor.getBoundingClientRect();
    var pw = __weColorPanel.offsetWidth || 280;
    var ph = __weColorPanel.offsetHeight || 200;
    ...
    if (top < 8) top = 8;
    __weColorPanel.style.left = left + 'px';
    __weColorPanel.style.top = top + 'px';
  }
}

// ★ 调原生接口实时应用 PMKG 属性修改
function wallpaperEngineApplyPropertyChange(key, value) {
  try {
    if (window.KeepApp && typeof window.KeepApp.applyMpkgSceneProperty === 'function') {
      window.KeepApp.applyMpkgSceneProperty(key, String(value));
    } else {
      showToast('当前环境不支持调整 PMKG 场景参数');
    }
  } catch (e) {
...
// ★ 重置当前 PMKG 壁纸的所有属性到默认值
function resetWallpaperEngineProjectDetails() {
  var id = wallpaperEngineProjectDetailsId;
  if (!id) return;
  var item = wallpaperEngineProjectById(id);
  if (!item || !window.KeepApp || typeof window.KeepApp.resetMpkgSceneProperties !== 'function') {
    showToast('当前环境不支持重置');
    return;
  }
  try {
    var rawJson = window.KeepApp.resetMpkgSceneProperties();
    var props = rawJson ? JSON.parse(rawJson) : [];
    if (!Array.isArray(props) || !props.length) {
      showToast('重置失败：当前没有可重置的场景参数');
      return;
    }
    var details = normalizeWallpaperEngineProjectDetails({
      id: id,
      title: item.title,
      projectType: 'scene',
      editable: true,
        ...
        return {
          ...
          options: Array.isArray(p.options)
            ? p.options.map(function (opt) {
                return { label: opt && opt.label, value: opt && opt.value };
              })
            : []
        };
      })
...
async function showWallpaperEngineProjectDetails(id) {
  ...
  // ★ PMKG 项目走原生接口 window.KeepApp.getMpkgSceneProperties()
  var item = wallpaperEngineProjectById(id);
  var isPmkg = item && item.source === 'local' && item.projectType === 'scene'
    && item.enginePlayable && item.safetyMode === 'native-engine';
  if (isPmkg) {
    try {
      if (!window.KeepApp || typeof window.KeepApp.getMpkgSceneProperties !== 'function') {
        throw new Error('当前环境不支持读取 PMKG 场景参数');
      }
      // ★ 检查：原生 getMpkgSceneProperties 读取的是当前已应用为背景的 PMKG 属性
      //   如果用户点击的不是当前背景卡片的⚙按钮，提示先应用为背景
      if (!wallpaperEngineSelection.active || wallpaperEngineSelection.id !== id) {
        renderWallpaperEngineProjectDetails(null, '请先将此壁纸设为背景后再调整设置（当前面板仅显示已应用为背景的壁纸参数）');
        return;
      }
      var rawJson = window.KeepApp.getMpkgSceneProperties();
      if (wallpaperEngineProjectDetailsId !== id) return;
      var props = rawJson ? JSON.parse(rawJson) : [];
      if (!Array.isArray(props) || !props.length) {
        renderWallpaperEngineProjectDetails(null, '请先将此壁纸设为背景后再调整设置');
        return;
      }
      var details = normalizeWallpaperEngineProjectDetails({
        id: id,
  ...
  // 普通 Wallpaper Engine 项目走 desktopApi
  var api = wallpaperEngineDesktopApi();
...
async function openWallpaperEngineLibrary() {
  ...
  // ★ Android WebView 无桌面桥接：loadWallpaperEngineLibrary 不会设置 snapshot，
  //   必须先兜底恢复本地 IndexedDB 项目并始终渲染，否则面板会空白/不显示已导入壁纸
  await restoreWallpaperEngineLocalProjects();
...
function pickWallpaperFileViaHtml5() {
  return new Promise(function (resolve) {
    try {
      var input = document.createElement('input');
      input.type = 'file';
      input.accept = '.pmkg,.pkg,.pkm,.mpkg,application/octet-stream';
      ...
      var finish = function (val) {
        if (settled) return;
        settled = true;
        try { input.onchange = null; } catch (e) {}
        try { window.removeEventListener('focus', onWinFocus, true); } catch (e) {}
...
function launchMpkgWallpaperNative(file) {
  var name = String(file && file.name || '');
  var path = '';
  try { path = String(file && (file.path || file.webkitRelativePath) || name); } catch (e) { path = name; }
  // ★ Android 原生桥接：KAppBridge 注册名为 "KeepApp"
  //   Android WebView 的 File 对象没有 path 属性（Electron 特性），
  //   因此传递文件名给原生层，原生层通过 onShowFileChooser 保存的 URI 读取文件内容。
  try {
    if (window.KeepApp && typeof window.KeepApp.launchMpkgPreview === 'function') {
      window.KeepApp.launchMpkgPreview(name || path);
      return Promise.resolve(true);
    }
  } catch (e) {}
  try {
    if (window.MR && typeof window.MR.launchMpkgPreview === 'function') {
      window.MR.launchMpkgPreview(path);
      return Promise.resolve(true);
    }
  } catch (e) {}
  try {
    if (window.AndroidBridge && typeof window.AndroidBridge.launchMpkgPreview === 'function') {
      window.AndroidBridge.launchMpkgPreview(path);
      return Promise.resolve(true);
    }
  } catch (e) {}
  showToast('PMKG 壁纸需要原生支持');
  return Promise.resolve(false);
}

var wallpaperEngineLocalMediaMap = new Map();

// ★ Android 本地壁纸持久化：导入的壁纸文件（PMKG/图片/视频）存入 IndexedDB，重启后自动恢复。
//   桌面版通过 desktopWindow 磁盘镜像持久化，Android WebView 无此桥接，只能靠 IndexedDB。
var WALLPAPER_ENGINE_LOCAL_DB_NAME = 'mineradio-wallpaper-engine-local-v1';
var WALLPAPER_ENGINE_LOCAL_STORE = 'media';
function openWallpaperEngineLocalDb() {
  return new Promise(function (resolve, reject) {
    if (!window.indexedDB) { reject(new Error('indexedDB unavailable')); return; }
    try {
      var req = indexedDB.open(WALLPAPER_ENGINE_LOCAL_DB_NAME, 1);
      req.onupgradeneeded = function () {
        var db = req.result;
        if (db && !db.objectStoreNames.contains(WALLPAPER_ENGINE_LOCAL_STORE)) db.createObjectStore(WALLPAPER_ENGINE_LOCAL_STORE, { keyPath: 'id' });
      };
...
function persistWallpaperEngineLocalProject(id, file, meta) {
  return new Promise(function (resolve) {
    openWallpaperEngineLocalDb().then(function (db) {
      var tx = db.transaction(WALLPAPER_ENGINE_LOCAL_STORE, 'readwrite');
      tx.objectStore(WALLPAPER_ENGINE_LOCAL_STORE).put(Object.assign({ id: id, blob: file, savedAt: Date.now() }, meta || {}));
      tx.oncomplete = function () { db.close(); resolve(true); };
      tx.onerror = function () { db.close(); resolve(false); };
    }).catch(function () { resolve(false); });
  });
}
function readWallpaperEngineLocalProjects() {
  return new Promise(function (resolve) {
    openWallpaperEngineLocalDb().then(function (db) {
      var tx = db.transaction(WALLPAPER_ENGINE_LOCAL_STORE, 'readonly');
      var req = tx.objectStore(WALLPAPER_ENGINE_LOCAL_STORE).getAll();
      req.onsuccess = function () { resolve(req.result || []); };
...
function removeWallpaperEngineLocalProject(id) {
  return new Promise(function (resolve) {
    openWallpaperEngineLocalDb().then(function (db) {
      var tx = db.transaction(WALLPAPER_ENGINE_LOCAL_STORE, 'readwrite');
      tx.objectStore(WALLPAPER_ENGINE_LOCAL_STORE).delete(id);
      tx.oncomplete = function () { db.close(); resolve(true); };
      tx.onerror = function () { db.close(); resolve(false); };
    }).catch(function () { resolve(false); });
  });
}
// 重启后从 IndexedDB 恢复本地导入的壁纸项目（含 blob URL）
async function restoreWallpaperEngineLocalProjects() {
  var records = await readWallpaperEngineLocalProjects();
  if (!records || !records.length) return;
  var existing = new Set(wallpaperEngineProjects.map(function (p) { return p.id; }));
  var added = 0;
  records.forEach(function (rec) {
    var id = String(rec.id || '');
    var blob = rec.blob;
    if (!id || id.length !== 24 || !blob || existing.has(id)) return;
    var mediaType = String(rec.mediaType || 'image');
    var blobUrl = '';
    try { blobUrl = URL.createObjectURL(blob); } catch (e) {}
    wallpaperEngineLocalMediaMap.set(id, { url: blobUrl, file: blob, name: String(rec.name || '') });
    var isPmkg = mediaType === 'pmkg';
    wallpaperEngineProjects.push({
      id: id,
      title: String(rec.title || rec.name || '本地壁纸').replace(/\.[^.]+$/, ''),
      projectType: isPmkg ? 'scene' : 'local',
      mediaType: isPmkg ? '' : mediaType,
      mediaAnimated: mediaType === 'video',
      playable: !isPmkg,
      enginePlayable: isPmkg,
      previewOnly: false,
      hasPreview: !!rec.previewData,
      previewAnimated: mediaType === 'video',
      previewData: rec.previewData || '',
      source: 'local',
      sourceLabel: isPmkg ? 'PMKG 场景' : '本地文件',
      workshopId: '',
      propertyCount: 0,
      audioPropertyCount: 0,
      mutedAudioPropertyCount: 0,
      updatedAt: Number(rec.savedAt) || Date.now(),
      safetyMode: isPmkg ? 'native-engine' : 'direct-media'
    });
    existing.add(id);
    added++;
  });
  if (added) renderWallpaperEngineLibrary();
}
// 重启后重新应用已保存的壁纸选择（PMKG 走原生预览，其余走 applyWallpaperEngineBackground）
async function applyStoredWallpaperEngineSelection() {
  // ★ 只有用户上次明确选择（active=true）才自动恢复；恢复原背景后即使 id 残留也不再自动重新选择壁纸
  if (!wallpaperEngineSelection.active) return false;
  var item = wallpaperEngineProjectById(wallpaperEngineSelection.id);
  if (!item) return false;
  if (item.source === 'local' && item.projectType === 'scene' && item.safetyMode === 'native-engine') {
    document.body.classList.add('wallpaper-engine-active');
    wallpaperEngineSelection.active = true;
    saveWallpaperEngineSelection();
    updateWallpaperEngineEntryUi(item.title + ' · 已应用');
    // 优先用 wallpaperSetAppBg 重新启动原生持久化的 MPKG 背景（重启后 lastPickedFileUri 已失效）
    try {
      if (window.KeepApp && typeof window.KeepApp.wallpaperSetAppBg === 'function') {
        window.KeepApp.wallpaperSetAppBg();
        return true;
      }
    } catch (e) { console.warn('[WE] restore wallpaperSetAppBg failed: ' + e); }
    // 回退：尝试 launchMpkgPreview（仅在当前会话 lastPickedFileUri 可用时有效）
    var fallbackEntry = wallpaperEngineLocalMediaMap.get(item.id);
    var pmkgName = (fallbackEntry && fallbackEntry.name) || item.title || '';
    try {
      if (window.KeepApp && typeof window.KeepApp.launchMpkgPreview === 'function') {
        window.KeepApp.launchMpkgPreview(pmkgName);
        return true;
      }
    } catch (e) { console.warn('[WE] restore pmkg preview failed: ' + e); }
    return true;
  }
  return applyWallpaperEngineBackground(item, true);
}

function addLocalWallpaperProject(file, mediaType) {
  if (!file) return '';
  // 生成 24 位 hex id
  var ts = Date.now().toString(16).padStart(12, '0');
  var rnd = Math.floor(Math.random() * 0xFFFFFFFFF).toString(16).padStart(12, '0');
  var id = (ts + rnd).slice(0, 24).toLowerCase();
  // 创建 blob URL 供预览使用
  var blobUrl = '';
  try { blobUrl = URL.createObjectURL(file); } catch (e) {}
  wallpaperEngineLocalMediaMap.set(id, { url: blobUrl, file: file, name: String(file.name || '') });
  // ★ PMKG 类型使用 scene + enginePlayable，其他类型使用 direct-media
  var isPmkg = mediaType === 'pmkg';
  // 创建项目对象
  var project = {
    id: id,
    title: String(file.name || '本地壁纸').replace(/\.[^.]+$/, ''),
    projectType: isPmkg ? 'scene' : 'local',
    mediaType: isPmkg ? '' : mediaType,
    mediaAnimated: mediaType === 'video',
    playable: !isPmkg,
  ...
  // ★ 去重：如果同标题的本地项目已存在，不重复添加
  var existingIdx = wallpaperEngineProjects.findIndex(function (p) { return p.title === project.title && p.source === 'local'; });
  if (existingIdx >= 0) {
    wallpaperEngineProjects.splice(existingIdx, 1);
  }
  wallpaperEngineProjects.push(project);
  // ★ 持久化到 IndexedDB，重启后自动恢复该本地壁纸项目
  try {
    persistWallpaperEngineLocalProject(id, file, {
      mediaType: mediaType,
      title: project.title,
      name: String(file.name || ''),
      previewData: ''
    });
  } catch (e) { console.warn('[WE] persist local wallpaper failed: ' + e); }
  renderWallpaperEngineLibrary();
  return id;
}

function handlePickedWallpaperFile(file) {
  if (!file) return Promise.resolve();
  var name = String(file.name || '');
  var mime = String(file.type || '');
  console.log('[WE Import] handlePickedWallpaperFile name="' + name + '" mime="' + mime + '" size=' + (file.size || 0));
  // ★ PMKG/PKG 壁纸：添加到 Wallpaper Engine 面板列表（用户点击列表项后再启动原生预览）
  if (/\.p?mkg$/i.test(name) || /\.pkg$/i.test(name) || /\.pkm$/i.test(name) || /\.mpkg$/i.test(name) ||
      (window.KeepApp && typeof window.KeepApp.launchMpkgPreview === 'function' && !mime)) {
    var pmkgCardId = addLocalWallpaperProject(file, 'pmkg');
    showToast('已添加到壁纸面板: ' + name);
    // ★ 参照 mineradio-android：调用原生 getWallpaperPreviewData 提取 PMKG 封面，让面板显示预览封面
    try {
      if (window.KeepApp && typeof window.KeepApp.getWallpaperPreviewData === 'function') {
        var pmkgCover = window.KeepApp.getWallpaperPreviewData(pmkgCardId, name);
        if (pmkgCover && pmkgCover.indexOf('data:image/') === 0) {
          var pmkgProj = wallpaperEngineProjectById(pmkgCardId);
          if (pmkgProj) pmkgProj.previewData = pmkgCover;
          // ★ 同步更新 IndexedDB，保证重启后卡片仍显示封面
          try {
            var mediaEntry = wallpaperEngineLocalMediaMap.get(pmkgCardId);
            if (mediaEntry && mediaEntry.file) {
              persistWallpaperEngineLocalProject(pmkgCardId, mediaEntry.file, {
                mediaType: 'pmkg',
                title: pmkgProj.title,
                name: String(mediaEntry.name || ''),
                previewData: pmkgCover
              });
            }
          } catch (e2) { console.warn('[WE] persist pmkg cover failed: ' + e2); }
          renderWallpaperEngineLibrary();
        }
      }
    } catch (e) { console.warn('[WE] extract pmkg cover failed: ' + e); }
    return Promise.resolve();
  }
  // ★ 仅支持 PMKG 壁纸：其它格式（视频/图片等）一律拒绝
  showToast('仅支持 PMKG 壁纸文件: ' + name);
  return Promise.resolve();
}
...
async function chooseWallpaperEngineDirectory() {
  ...
  // Android WebView：打开文件管理器选择视频/图片/PMKG 壁纸
  var file = await pickWallpaperFileViaHtml5();
  if (file) await handlePickedWallpaperFile(file);
}
...
function initializeWallpaperEngineLibrary() {
  ...
  updateWallpaperEngineEntryUi();
  // ★ 从 IndexedDB 恢复本地导入的壁纸项目，并重新应用已保存的壁纸选择
  restoreWallpaperEngineLocalProjects().then(function () {
    return applyStoredWallpaperEngineSelection().then(function (restored) {
      if (restored) {
        updateWallpaperEngineEntryUi();
        return;
      }
