...
function triggerPresetParticleTransition(fromPreset, toPreset) {
  ...
  // ★ 减弱预设切换时的粒子爆发与镜头冲击，避免透过玻璃面板让整组预设卡片连片发亮
  uniforms.uScatter.value = Math.max(uniforms.uScatter.value, fx.scatter + (newVisual ? (wallpaperFlow ? 0.004 : 0.012) : 0.05));
  uniforms.uBurstAmt.value = Math.max(uniforms.uBurstAmt.value, wallpaperFlow ? 0.03 : 0.07);
  camPunch = Math.max(camPunch, wallpaperFlow ? 0.02 : 0.05);
  for (var i = 0; i < 3; i++) {
...
function tickPresetTransition() {
  ...
  // ★ 同步减弱过渡过程中的粒子爆发幅度
  uniforms.uScatter.value = Math.max(uniforms.uScatter.value, fx.scatter + wave * (newVisual ? (wallpaperFlow ? 0.004 : 0.012) : 0.06));
  uniforms.uBurstAmt.value = Math.max(uniforms.uBurstAmt.value, wave * (wallpaperFlow ? 0.025 : (newVisual ? 0.05 : 0.06)));
  uniforms.uPointScale.value = fx.point * (1 + wave * (wallpaperFlow ? 0.008 : 0.02));
  if (raw >= 1) {
