...
function animate() {
  ...
  // 缩略图脉动(帧门控 + 值diff, 避免每帧无条件写DOM)
  if (currentIdx >= 0 && (now - lastThumbCoverWriteAt) > 50) {
    var s = 1 + bass * 0.08;
    if (Math.abs(s - lastThumbCoverScale) > 0.001) {
      lastThumbCoverScale = s;
      lastThumbCoverWriteAt = now;
