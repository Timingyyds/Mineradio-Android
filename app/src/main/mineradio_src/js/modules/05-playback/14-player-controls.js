...
async function completeAudioPlayStart(opts, reason, expectedMedia, expectedToken) {
  ...
  forcePlaybackControlsInteractive();
  hideLoading();
  // ★ 通知 Android 启动前台音乐播放服务（显示通知栏）
  try {
    if (typeof KeepApp !== 'undefined' && KeepApp && typeof KeepApp.setMusicPlaying === 'function') {
      KeepApp.setMusicPlaying(true);
    }
  } catch (e) { /* KeepApp 仅在 Android WebView 中可用 */ }
  return true;
}
...
function withPlaybackToggleTimeout(promise, ms) {
  ms = Math.max(3000, Number(ms) || 25000);
  return new Promise(function (resolve) {
    var settled = false;
    var timer = setTimeout(function () {
      if (settled) return;
...
async function togglePlay() {
  if (playToggleBusy) {
    // 上一次点击仍在尝试(等待本地 server / 音源解析),给出即时反馈而不是静默吞掉
    showToast('正在处理播放请求，请稍候…');
    return;
  }
  playToggleBusy = true;
  try {
    ...
    if ((!audio || !audio.src) && playQueue.length && currentIdx >= 0) {
      await withPlaybackToggleTimeout(playQueueAt(currentIdx, { manual: true }));
      return;
    }
    if (audio && audio.src && playQueue.length && currentIdx >= 0 && !playbackMediaMatchesCurrentQueueItem(audio)) {
      await withPlaybackToggleTimeout(playQueueAt(currentIdx, { manual: true, suppressPlayFailureNotice: true }));
      return;
    }
    ...
    if (!playQueue.length && !currentLocalSong) {
      showToast('队列为空，请先选择一首歌曲');
      return;
    }
    if (!audio) return;
    if (audio.paused || audio.ended) {
      await withPlaybackToggleTimeout(attemptAudioPlay({ manual: true }));
    } else {
...
function clearQueue() {
  console.log('[AutoplayDiag] SENTINEL clearQueue');
...
// ════════════════════════════════════════════════════════════════
//  ★ Android 通知栏/锁屏控制器回调
//  由 Android MediaNotificationState → LandscapeWebActivity.registerNotificationCallbacks
//  通过 evaluateJavascript 调用以下全局函数
// ════════════════════════════════════════════════════════════════

// 通知栏 播放/暂停 按钮
function _onNotificationPlayPause() {
  try { if (typeof togglePlay === 'function') togglePlay(); }
  catch (e) { console.warn('[NotifPlayPause]', e); }
}

// 通知栏 上一首 按钮
function _onNotificationPrev() {
  try { if (typeof prevTrack === 'function') prevTrack(); }
  catch (e) { console.warn('[NotifPrev]', e); }
}

// 通知栏 下一首 按钮
function _onNotificationNext() {
  try { if (typeof nextTrack === 'function') nextTrack(); }
  catch (e) { console.warn('[NotifNext]', e); }
}

// 通知栏进度条拖动 Seek 回调 — seekSec 为目标位置（秒）
function handleMediaSeek(seekSec) {
  if (!audio || !isFinite(seekSec)) return;
  try {
    var dur = audio.duration || 0;
    var target = dur > 0 ? Math.max(0, Math.min(seekSec, dur - 0.1)) : seekSec;
    audio.currentTime = target;
    if (typeof setProgressVisual === 'function') {
      setProgressVisual(dur > 0 ? (target / dur * 100) : 0);
    }
    if (typeof updatePlaybackProgressUi === 'function') updatePlaybackProgressUi();
  } catch (e) { console.warn('[NotifSeek]', e); }
}
