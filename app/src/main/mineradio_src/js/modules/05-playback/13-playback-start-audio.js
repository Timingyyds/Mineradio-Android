...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    currentIdx = idx;
    console.log('[AutoplayDiag] SENTINEL playQueueAt set currentIdx=' + idx);
    trackSwitchToken++;
    ...
    safePlaybackStep('track-ui', function () {
      ...
      // ★ 同步歌曲元数据到 Android 通知栏（标题/艺术家/封面/时长）
      try {
        if (typeof KeepApp !== 'undefined' && KeepApp && typeof KeepApp.updateMediaTrack === 'function') {
          KeepApp.updateMediaTrack(JSON.stringify({
            title: song.name || '',
            artist: (song.artist || '').replace(/\s*\/\s*/g, ', '),
            album: song.album || '',
            cover: song.cover ? coverUrlWithSize(song.cover, 400) : '',
    ...
    try {
      ...
      // 移动端:本地 Node server 未就绪时 fetch 会拿到 503 降级响应,
      // 播放尝试会整条空转(换源 / 重试)后才失败;这里先等 server 就绪再请求,
      // 保证重启后的自动播放与手动点击都能在 server 可用时立即成功。
      if (typeof waitForLocalServerReady === 'function') {
        await waitForLocalServerReady(20000);
        if (token !== trackSwitchToken) return false;
      }
      var playbackProvider = normalizePlaybackProvider(songProviderKey(song));
      ...
      if (data.trial && isQishuiPlayback && !opts.fallbackDepth) {
        // 汽水返回的是试听片段：先尝试从其它已登录平台获取完整版，避免只听 30 秒试听。
        var trialFallbackOpts = Object.assign({}, retryPlaybackOpts, { keepTrialAsFallback: true });
        var trialFallbackResult = await tryAutoPlaybackFallback(song, data, idx, token, trialFallbackOpts);
        if (trialFallbackResult === true) return true;
      }
      if (data.trial) {
        var txt;
        // ★ 已确认会员身份却仍拿到试听片段时，不能再提示「需要开通 VIP」，
        //   否则用户会以为是会员没生效。此时应说明是音源侧限制，并引导换源。
        var accountIsVip = !!(data.loggedIn && (data.isVip || data.isSvip || data.vipLevel === 'vip' || data.vipLevel === 'svip'));
        if (accountIsVip) {
          txt = '已识别到会员权益，但汽水本次只返回了试听音源 · 可换源收听完整版';
        ...
        if (trialLoginBtn) {
          // 会员账号不需要再点「登录」，改为不显示该按钮，避免误导。
          trialLoginBtn.style.display = (data.loggedIn || accountIsVip) ? 'none' : '';
