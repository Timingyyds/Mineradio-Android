...
var audioRouteWorkflowEdgeRenderFrame = 0;
var audioRouteWorkflowEdgeRenderTimers = [];
// 与登录面板连线一致:以根节点 getBoundingClientRect 视觉坐标为准,
...
function renderAudioRouteWorkflowEdgesForRoot(root, tempPoint) {
  ...
  if (!svg) return;
  // 与登录面板一致:viewBox 取根节点视觉尺寸,保证 SVG 用户坐标与视觉像素 1:1
  var rootRect = root.getBoundingClientRect();
...
function scheduleAudioRouteWorkflowEdges(reason) {
  if (audioRouteWorkflowEdgeRenderFrame) cancelAnimationFrame(audioRouteWorkflowEdgeRenderFrame);
  audioRouteWorkflowEdgeRenderFrame = requestAnimationFrame(function () {
    audioRouteWorkflowEdgeRenderFrame = 0;
    renderAudioRouteWorkflowEdges();
  });
  audioRouteWorkflowEdgeRenderTimers.forEach(function (timer) { clearTimeout(timer); });
  audioRouteWorkflowEdgeRenderTimers = [];
  [70, 170, 340, 560].forEach(function (delay) {
    audioRouteWorkflowEdgeRenderTimers.push(setTimeout(function () {
      renderAudioRouteWorkflowEdges();
    }, delay));
  });
}
function finishAudioRouteWorkflowDrag(e) {
  ...
  scheduleAudioRouteWorkflowEdges('drag-finish');
}
function bindAudioRouteWorkflowPointerEvents(outputList) {
  ...
  outputList.addEventListener('pointercancel', function (e) {
    ...
    scheduleAudioRouteWorkflowEdges('pointer-cancel');
  });
  if (!bindAudioRouteWorkflowPointerEvents._resizeBound) {
    bindAudioRouteWorkflowPointerEvents._resizeBound = true;
    window.addEventListener('resize', function () { scheduleAudioRouteWorkflowEdges('resize'); });
    window.addEventListener('orientationchange', function () { scheduleAudioRouteWorkflowEdges('orientation'); });
    window.addEventListener('scroll', function () { scheduleAudioRouteWorkflowEdges('scroll'); }, true);
  }
}
...
function markAudioOutputMirrorRuntime(id, state, message) {
  ...
  schedule(function () {
    ...
    renderAudioOutputDeviceUi();
    scheduleAudioRouteWorkflowEdges('mirror-runtime');
  });
}
...
function renderAudioOutputDeviceUi() {
  ...
  scheduleAudioRouteWorkflowEdges('workflow-ui');
}
function openAudioOutputWorkflowPanel() {
  ...
  renderAudioOutputDeviceUi();
  scheduleAudioRouteWorkflowEdges('open');
  refreshAudioOutputDevices(false);
}
