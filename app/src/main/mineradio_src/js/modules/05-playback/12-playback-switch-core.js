...
function syncPlaybackStateFromAudioEvent(reason) {
  ...
  // ★ 同步播放/暂停状态到 Android 通知栏（MediaNotificationState）
  try {
    if (typeof KeepApp !== 'undefined' && KeepApp && typeof KeepApp.updateMediaPlayState === 'function') {
      KeepApp.updateMediaPlayState(isPlaying);
    }
...
function scheduleAudioResumePosition(media, seconds, token) {
  ...
  function applyResume() {
    ...
    if (duration > 0 && seconds >= duration - 0.5) return;
