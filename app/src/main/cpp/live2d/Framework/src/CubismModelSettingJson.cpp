...
CubismModelSettingJson::CubismModelSettingJson(const csmByte* buffer, csmSizeInt size)
{
    ...
    {
        // ★ 调试：解析失败时直接用 android log 打印错误信息
        __android_log_print(ANDROID_LOG_ERROR, "MRPET", "[CubismModelSettingJson] JSON parse FAILED. size=%d", size);
        if (buffer && size > 0)
        {
            // 打印前 80 字节作为字符串（截断非打印字符）
            char strbuf[100] = {0};
            int n = (size < 80) ? size : 80;
            for (int k = 0; k < n; k++)
            {
                char c = buffer[k];
                strbuf[k] = (c >= 32 && c < 127) ? c : '.';
            }
            __android_log_print(ANDROID_LOG_ERROR, "MRPET", "[CubismModelSettingJson] first %d chars: %s", n, strbuf);
            ...
            __android_log_print(ANDROID_LOG_ERROR, "MRPET", "[CubismModelSettingJson] hex: %s", hexbuf);
        }
        else
        {
            __android_log_print(ANDROID_LOG_ERROR, "MRPET", "[CubismModelSettingJson] buffer=NULL or size=0");
        }
    }
}
