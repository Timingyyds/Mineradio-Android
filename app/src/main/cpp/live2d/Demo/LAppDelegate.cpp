...
namespace {
    ...
    // ★ 修复闪退：全局标志，标记 native 资源是否已释放
    // 防止 OnStop 被多次调用导致重复释放崩溃
    bool g_nativeReleased = false;
}
...
{
    // ★ 修复闪退：防止重复释放（多次调用 OnStop 时跳过）
    if (g_nativeReleased) return;
    ...
    g_nativeReleased = true;
}

// ★ 全局函数：重置 OnStop 的释放标志（在新 GL Surface 创建时调用）
extern "C" void ResetReleaseFlag()
{
    g_nativeReleased = false;
}
...
{
    // ★ 修复闪退：每次 Surface 创建时重置释放标志（允许重新初始化 native 资源）
    extern void ResetReleaseFlag();
    ResetReleaseFlag();
