...
void LAppView::InitializeSprite()
{
    ...
    // 背景 sprite（容错: 文件不存在则跳过，不创建 _back）
    {
    ...
    // 齿轮 sprite（容错: 文件不存在则跳过）
    {
        string imageName = GearImageName;
        LAppTextureManager::TextureInfo* gearTexture = textureManager->CreateTextureFromPngFile(resourcesPath + imageName);
        if (gearTexture != NULL)
        {
            float x = (width - gearTexture->width * 0.5f - 96.f);
            float y = (height - gearTexture->height * 0.5f);
            float fWidth = static_cast<float>(gearTexture->width);
            float fHeight = static_cast<float>(gearTexture->height);
    ...
    // 电源按钮 sprite（容错: 文件不存在则跳过）
    {
        ...
        {
            float x = (width - powerTexture->width * 0.5f - 96.f);
            float y = (powerTexture->height * 0.5f);
