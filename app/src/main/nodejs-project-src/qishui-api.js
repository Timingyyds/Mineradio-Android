...
function qishuiSessionCookieHeader(cookieText) {
  // 参考项目逻辑: 只发纯 sessionid 规避风控, 发完整 cookie 会触发试听版/API异常
...
async function qishuiWebRequestJson(apiPath, params, cookieText, opts) {
  ...
  for (const base of bases) {
    ...
    try {
      const headers = qishuiWebHeaders(cookieText, opts);
      if (body) {
      ...
      lastErr = err;
      if (/track_v2/.test(apiPath)) {
        try {
          console.log('[QishuiTrackV2Diag]', method, targetUrl, 'cookie=' + (headers && headers.Cookie || ''), 'errcode=' + (err && err.statusCode) + ' errmsg=' + (err && err.message) + ' body=' + String((err && err.body) || '').slice(0, 400));
        } catch (_) {}
      }
...
async function fetchQishuiPlaybackMembership(cookieText) {
  ...
  return qishuiMembershipCache.wrap(cacheKey, qishuiMembershipCacheTtlMs, async () => {
    try {
      const json = await qishuiWebRequestJson('/luna/pc/me', qishuiWebCommonParams(), cookie, {
...
async function qishuiPcPostJson(apiPath, payload, cookieText, opts) {
  ...
  const body = JSON.stringify(payload || {});
  const json = await requestJson(qishuiPcUrl(apiPath, qishuiWebCommonParams(opts.params)), {
    method: 'POST',
    timeoutMs: opts.timeoutMs || 9000,
    headers: Object.assign(qishuiWebHeaders(cookie, { sessionOnly: true }), {
      'Content-Length': Buffer.byteLength(body),
  ...
  return qishuiWebLibraryCache.wrap(cacheKey, 90 * 1000, async () => {
    ...
    const tryRead = async (label, apiPath, params, requestOpts) => {
      ...
      try {
        ...
        console.log('[QishuiLibrary] tryRead ERROR label=' + label + ' err=' + (err && err.message || err));
        if (!requestOpts.optional) errors.push(label + ':' + (err && err.message || 'failed'));
        return null;
      }
    };

    // 改用 Web 端点(device_platform=web): music.douyin.com 获取的是 Web sessionid,
    // 不能用于 PC App 端点(device_platform=windows), 否则返回 ERR_REQUEST_FORBIDDEN
    const webRequestOpts = { sessionOnly: true };
    // 调试: 输出cookie中sessionid相关字段
    const cookieObj = qishuiCookieObject(cookie);
    console.log('[QishuiLibrary] cookieFields: sessionid=' + (cookieObj.sessionid ? 'YES(len=' + cookieObj.sessionid.length + ')' : 'NO') + ' sessionid_ss=' + (cookieObj.sessionid_ss ? 'YES' : 'NO') + ' sid_guard=' + (cookieObj.sid_guard ? 'YES' : 'NO') + ' uid_tt=' + (cookieObj.uid_tt ? 'YES' : 'NO'));
    console.log('[QishuiLibrary] sessionCookieHeader=' + qishuiSessionCookieHeader(cookie).substring(0, 100));
    const meJson = await tryRead('me', '/luna/pc/me', qishuiWebCommonParams(), webRequestOpts);
    console.log('[QishuiLibrary] meJson=' + (meJson ? JSON.stringify(meJson).substring(0, 500) : 'null'));
    const meData = (meJson && meJson.data) || meJson || {};
    const profile = qishuiProfileFromMeData(meData);
    const userId = profile.userId;
    console.log('[QishuiLibrary] profile=' + JSON.stringify({ userId: profile.userId, nickname: profile.nickname, avatar: profile.avatar ? profile.avatar.substring(0, 50) : '', vipLevel: profile.vipLevel }));

    await Promise.all([
      userId
        ? tryRead('created', '/luna/pc/user/playlist', qishuiWebCommonParams({
          user_id: userId,
          cursor: '',
          count: 50,
        }), webRequestOpts)
        : Promise.resolve(null),
      tryRead('collection', '/luna/pc/me/collection/mixed', qishuiWebCommonParams({
        cursor: '',
        count: 50,
      }), Object.assign({ optional: true }, webRequestOpts)),
      tryRead('recent', '/luna/pc/me/recently-played-media', qishuiWebCommonParams({
        cursor: '',
        count: 50,
      }), Object.assign({ optional: true }, webRequestOpts)),
    ]);
...
async function handleQishuiStatus(cookieText) {
  ...
  try {
    try {
      const cookieObj = qishuiCookieObject(normalizeQishuiCookieInput(cookieText));
      status.cookieKeys = Object.keys(cookieObj || {}).join(',');
      status.hasPassportCsrf = !!(cookieObj && cookieObj.passport_csrf_token);
      status.hasSidGuard = !!(cookieObj && (cookieObj.sid_guard || cookieObj.sid_tt));
      status.hasUidTt = !!(cookieObj && cookieObj.uid_tt);
    } catch (_) {}
  ...
  return qishuiWebPlaylistCache.wrap(cacheKey, 90 * 1000, async () => {
    ...
    while (cursorState.rawItems.length < targetCount && cursorState.hasMore) {
      if (!cursorState.promise) {
        cursorState.promise = qishuiWebRequestJson('/luna/pc/playlist/detail', qishuiWebCommonParams({
          playlist_id: id,
...
async function handleQishuiPcSearch(keywords, limit, cookieText, offset) {
  ...
  offset = Math.max(0, Number(offset) || 0);
  const json = await qishuiWebRequestJson('/luna/pc/search/track', qishuiWebCommonParams({
    q: keywords,
...
async function handleQishuiComments(trackId, opts, cookieText) {
  ...
  const json = await qishuiWebRequestJson('/luna/pc/comments', qishuiWebCommonParams({
    group_id: id,
    cursor,
    count,
    group_type: 0,
...
async function fetchQishuiPcTrackV2Post(trackId, cookieText) {
  ...
  const json = await qishuiWebRequestJson('/luna/pc/track_v2', qishuiWebCommonParams(), cookieText, {
    bases: [QISHUI_WEB_PC_API_BASE],
    noDefaultParams: true,
    sessionOnly: true,
    method: 'POST',
...
async function fetchQishuiPcTrackV2Get(trackId, cookieText) {
  const json = await qishuiWebRequestJson('/luna/pc/track_v2', qishuiWebCommonParams({
    track_id: trackId,
    media_type: 'track',
  }), cookieText, {
    bases: [QISHUI_WEB_PC_API_BASE],
...
async function fetchQishuiPlayerInfo(playerInfoUrl, cookieText, membership) {
  ...
  // ★ 不能只依赖「会员过滤后的结果是否为空」来判断降级。
  //   会员状态未知（/luna/pc/me 超时或失败）时，qishuiRequiredTierAllowed 对
  //   free 档恒为 true，于是「会员过滤」会稳稳返回那条免费试听流；
  //   已开通会员的账号因此被静默降级成试听，前端随即弹「仅播放试听片段」。
  //   这里改为显式比较：会员状态未知时，若未受限的最优音源比会员过滤结果
  //   更优（通常是时长更长/音质更高的完整音源），就采用未受限的音源。
  const restricted = qishuiBestStreamCandidateForMembership(streams, membership);
  const unrestricted = qishuiBestStreamCandidate(streams);
  if (unrestricted && !(membership && membership.membershipKnown)) {
    if (!restricted || qishuiBetterStreamCandidate(unrestricted, restricted)) {
      return unrestricted;
    }
  }
...
async function resolveQishuiDownloadInfo(trackId, payload, cookieText, membership) {
  ...
  // ★ 与 fetchQishuiPlayerInfo 同源的处理：会员状态未知时必须显式比较，
  //   否则「会员过滤」会稳稳返回 free 试听档，把已开通会员的账号静默降级。
  const unrestrictedAll = qishuiBestStreamCandidate(collected.streams) ||
    qishuiBestStreamCandidate(collected.fallbackStreams);
  const membershipUnknown = !(membership && membership.membershipKnown);
  let best = qishuiBestStreamCandidateForMembership(collected.streams, membership) ||
    qishuiBestStreamCandidateForMembership(collected.fallbackStreams, membership);
  if (membershipUnknown && unrestrictedAll && (!best || qishuiBetterStreamCandidate(unrestrictedAll, best))) {
    best = unrestrictedAll;
  }
  if (best) return Object.assign(collected, { best });
  const unrestricted = unrestrictedAll;
...
// 判定汽水返回的音源是否为「试听片段」。
//
// 背景：早期实现直接用 `stream.duration + 5 < track.duration` 判断试听，
// 只要字面时长不一致就判定为试听，于是已开通会员的账号也会被标记成
// trial=true，前端随即弹「仅播放试听片段」横幅并把播放截断——即用户反馈的
// 「VIP 账号只能听试听部分」。
//
// 正确的判定需要同时满足三个条件：
//   1. 流时长确实明显短于整曲（排除 duration 字段缺失/单位换算误差）；
//   2. 当前账号没有覆盖该音质所需的权益（free 账号才可能真的只拿到试听）；
//   3. 该音源本身不是「账号已获授权」的音质档位。
// 只要账号权益已覆盖所需档位，就不能把结果标记为试听，应当按完整音源播放。
function qishuiResolveTrial(stream, track, membership) {
  const streamDuration = qishuiNormalizeDurationSeconds(
    (stream && stream.duration) || (track && (track.duration_ms || track.duration)) || 0
  );
  const fullDuration = qishuiNormalizeDurationSeconds(
    (track && (track.duration_ms || track.duration)) || 0
  );
  // 时长数据缺失时不能瞎猜：宁可当作完整音源，也不能误报试听把播放截断。
  if (!(streamDuration > 0 && fullDuration > 0)) return false;
  // 时长相差 5 秒以内视为同一版本（变速/编码截断误差），不算试听。
  if (!(streamDuration + 5 < fullDuration)) return false;
  // 到这里说明流确实短于整曲。若账号权益已经覆盖该音源档位，
  // 说明这是服务端针对第三方接入给出的降级音源，而不是权限不足的试听，
  // 不应让前端弹试听横幅。
  const requiredTier = qishuiStreamRequiredTier(stream);
  if (qishuiRequiredTierAllowed(requiredTier, membership)) {
    // 已登录且会员状态已知，且档位在权益范围内 → 不判定为试听。
    if (membership && membership.membershipKnown && (membership.isVip || membership.isSvip)) {
      return false;
    }
  }
  return true;
...
// 汽水官方已将 /luna/pc/track_v2 对第三方接入关闭（返回 200 空 body）。
// 回退到免登录的 seo_track 接口，其返回的 track_player.url_player_info
// 是 VOD GetPlayInfo 签名地址，可换取可直接播放的标准 M4A 音源。
async function resolveQishuiSeoPlayback(id, cookie, requestedQuality) {
  let seoPayload;
  try {
    seoPayload = await fetchQishuiSeoTrack(id);
  } catch (e) {
    return { error: 'Qishui did not return track playback metadata: ' + (e && e.message || String(e)) };
  }
  const player = (seoPayload && seoPayload.track_player) || {};
  const urlPlayerInfo = normalizeText(player.url_player_info || '');
  if (!urlPlayerInfo) return { error: 'Qishui did not return a playable audio source (no player info).' };
  let membership = qishuiUnknownMembership('QISHUI_MEMBERSHIP_CHECK_FAILED');
  try { membership = await fetchQishuiPlaybackMembership(cookie); } catch (_) {}
  let stream;
  try {
    stream = await fetchQishuiPlayerInfo(urlPlayerInfo, cookie, membership);
  } catch (e) {
    return { error: 'Qishui audio source failed: ' + (e && e.message || String(e)) };
  }
  if (!stream) return { error: 'Qishui did not return a playable audio source.' };
  const track = (seoPayload && (seoPayload.track || seoPayload.seo_track)) || {};
  ...
  // ★ 用权益感知的判定替代裸时长比较，避免 VIP 账号被误标试听
  const trial = qishuiResolveTrial(stream, track, membership);
  return {
    ok: true,
    result: {
      provider: 'qishui',
...
async function handleQishuiSongUrl(opts, cookieText) {
  ...
  try {
    ...
    // track_v2 接口已被汽水官方对第三方关闭（返回空响应），回退到免登录 seo_track 接口
    const seo = await resolveQishuiSeoPlayback(id, cookie, requestedQuality);
    if (seo.ok) return seo.result;
    return qishuiUnavailable(String(seo.error || 'Qishui playback failed'), 'source_unavailable', {
      loggedIn: true,
      playbackKeyReady: false,
      rawError: (err && err.message || String(err)) + ' | ' + String(seo.error || ''),
    });
  }
