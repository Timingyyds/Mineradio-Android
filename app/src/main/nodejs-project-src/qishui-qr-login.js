...
function createQishuiQrLoginBridge(options) {
  ...
  return {
    createQrCode,
    checkQrConnect,
    buildCheckQrConnectPayload: (token) => auth.buildCheckQrConnectPayload(token),
    processCheckQrConnectResponse: (responseBody) => auth.processCheckQrConnectResponse(responseBody),
    extractMfaDecision: (envelope) => auth.extractMfaDecision(envelope),
    extractBizParams: (envelope) => auth.extractBizParams(envelope),
    checkQrConnectResend: (token, bizParams) => auth.checkQrConnectResend(token, bizParams),
    getCookie,
    getStatus,
    clear,
