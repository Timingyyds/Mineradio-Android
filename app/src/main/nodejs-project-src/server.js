...
// Android 平台检测:无 C/D 盘概念,跳过 Windows 盘符检查
const IS_ANDROID_PLATFORM = typeof process !== 'undefined' && process.platform === 'android';
...
function beatCacheRootInfo() {
  ...
  // Android 平台:无盘符概念,直接允许使用配置的路径
  const allowed = IS_ANDROID_PLATFORM ? true : (!!root && !/^C:$/i.test(drive));
...
const server = http.createServer(async (req, res) => {
  ...
  // Android 缓存统计 API: 扫描外部存储目录大小(用户可访问,无需 root)
  if (pn === '/api/cache/storage-stats') {
    try {
      var osMod = require('os');
      // 使用 MINERADIO_CACHE_ROOT 环境变量(由 start.js 设置为外部存储目录)
      var cacheRoot = process.env.MINERADIO_CACHE_ROOT || (IS_ANDROID_PLATFORM ? '/storage/emulated/0/Android/data/com.mineradio.desktop/files/Mineradio' : BEATMAP_CACHE_DIR);
      var lyricsDir = process.env.MINERADIO_LYRIC_CACHE_DIR || path.join(cacheRoot, 'lyrics');
      var beatmapsDir = process.env.MINERADIO_BEAT_CACHE_DIR || path.join(cacheRoot, 'beatmaps');
      var coversDir = process.env.MINERADIO_COVER_CACHE_DIR || path.join(cacheRoot, 'covers');
      var audioDir = process.env.MINERADIO_AUDIO_CACHE_DIR || path.join(cacheRoot, 'audio');
      var chromiumDir = path.join(cacheRoot, 'chromium');
      var wallpaperDir = path.join(cacheRoot, 'wallpaper-engine');
      var userdataDir = path.join(cacheRoot, 'userdata');
      ...
      var lyricsBytes = dirSizeBytes(lyricsDir);
      var beatmapsBytes = dirSizeBytes(beatmapsDir);
      var coversBytes = dirSizeBytes(coversDir);
      var audioBytes = dirSizeBytes(audioDir);
      var chromiumBytes = dirSizeBytes(chromiumDir);
      var wallpaperBytes = dirSizeBytes(wallpaperDir);
      var userdataBytes = dirSizeBytes(userdataDir);
      var totalBytes = lyricsBytes + beatmapsBytes + coversBytes + audioBytes + chromiumBytes + wallpaperBytes + userdataBytes;

      sendJSON(res, {
        ok: true,
        settings: {
          rootPath: cacheRoot,
          lyricsPath: lyricsDir,
          beatmapsPath: beatmapsDir,
          activeBeatmapsPath: beatmapsDir,
          coversPath: coversDir,
          audioPath: audioDir,
          chromiumPath: chromiumDir,
          activeChromiumPath: chromiumDir,
          wallpaperEnginePath: wallpaperDir,
          activeWallpaperEnginePath: wallpaperDir,
          userDataPath: userdataDir,
          restartRequired: false
        },
        usage: {
          totalManagedBytes: totalBytes,
          lyricsBytes: lyricsBytes,
          coversBytes: coversBytes,
          audioBytes: audioBytes,
          beatmapsBytes: beatmapsBytes,
          chromiumBytes: chromiumBytes,
          wallpaperEngineBytes: wallpaperBytes,
          userDataBytes: userdataBytes
        }
      });
    } catch (err) {
      sendJSON(res, { ok: false, error: err.message || 'CACHE_STATS_FAILED' });
    }
    return;
  }

  // Android 内存信息 API: 返回系统和进程内存状态
  if (pn === '/api/system/memory') {
    try {
      var osMod2 = require('os');
      var procMem = process.memoryUsage();
      var totalMem = osMod2.totalmem();
      var freeMem = osMod2.freemem();
      ...
      sendJSON(res, { ok: false, error: err.message || 'MEMORY_READ_FAILED' });
    }
    return;
  }
  ...
  if (pn === '/api/qishui/login/check') {
    try {
      ...
      const signerReady = String(url.searchParams.get('signer_ready') || '').trim() === '1';
      if (!token) {
        sendJSON(res, {
          provider: 'qishui',
          loggedIn: false,
      ...
      // 签名桥接就绪时,直接返回 payload 供前端通过签名 WebView 发出请求(带 a_bogus),避免频率限制
      if (signerReady) {
        try {
          const signPayload = qishuiQrLogin.buildCheckQrConnectPayload(token);
          console.log('[QishuiLoginCheck] signer_ready, returning payload directly');
          sendJSON(res, {
            provider: 'qishui',
            loggedIn: false,
            needSign: true,
          ...
          console.warn('[QishuiLoginCheck] buildSignPayload failed for signer_ready:', e.message);
          // 降级: 走正常请求流程
        }
      }
      ...
      // 处理二次验证(MFA/2046): 返回 MFA 决策数据,由前端通过签名 WebView 显示验证面板
      if (errorCode === 2046) {
        try {
          const mfaDecision = qishuiQrLogin.extractMfaDecision(result);
          const bizParams = qishuiQrLogin.extractBizParams(result);
          console.log('[QishuiLoginCheck] 2046 MFA required, sending to frontend');
          sendJSON(res, {
            provider: 'qishui',
            loggedIn: false,
            needMfa: true,
            mfaDecision: mfaDecision,
            bizParams: bizParams,
            message: '需要二次验证'
          });
          return;
        } catch (mfaErr) {
          console.warn('[QishuiLoginCheck] extractMfaDecision failed:', mfaErr.message);
          // 降级: 返回错误状态
          sendJSON(res, {
            provider: 'qishui',
            loggedIn: false,
            status: 'mfa_error',
            errorCode: 2046,
            message: '二次验证数据解析失败: ' + mfaErr.message
          });
          return;
        }
      }
      ...
      if (errorCode === 2) status = 'expired';
        ...
        // 缺少安全签名(a_bogus),返回 payload 供前端通过签名 WebView 桥接重试
        try {
          const signPayload = qishuiQrLogin.buildCheckQrConnectPayload(token);
          sendJSON(res, {
            provider: 'qishui',
          ...
          console.warn('[QishuiLoginCheck] buildSignPayload failed, fallback to rate_limited:', e.message);
          status = 'rate_limited';
        }
      }
  ...
  // 前端通过签名 WebView 桥接获取签名后的响应,回传给后端处理
  if (pn === '/api/qishui/login/check/result') {
    try {
      const parsed = await readRequestBody(req);
      const token = String((parsed && parsed.token) || '').trim();
      const responseBody = String((parsed && parsed.responseBody) || '');
      const envelope = qishuiQrLogin.processCheckQrConnectResponse(responseBody);
      const data = envelope && envelope.data || {};
      const errorCode = Number(data.error_code || 0);

      // 处理二次验证(MFA/2046): 签名后的请求仍可能返回2046,需要前端处理
      if (errorCode === 2046) {
        try {
          const mfaDecision = qishuiQrLogin.extractMfaDecision(envelope);
          const bizParams = qishuiQrLogin.extractBizParams(envelope);
          console.log('[QishuiLoginCheckResult] 2046 MFA required after signed request');
          sendJSON(res, {
            provider: 'qishui',
            loggedIn: false,
            needMfa: true,
          ...
          console.warn('[QishuiLoginCheckResult] extractMfaDecision failed:', mfaErr.message);
          sendJSON(res, {
            provider: 'qishui',
            loggedIn: false,
            status: 'mfa_error',
      ...
      if (Number(data.error_code) === 0 && (String(data.status) === '3' || String(data.status) === 'confirmed' || data.session_cookie)) {
        const cookie = qishuiQrLogin.getCookie ? qishuiQrLogin.getCookie() : '';
      ...
      sendJSON(res, {
        ...
        errorCode,
        error_code: errorCode,
        retryAfterMs: errorCode === 7 ? 15000 : 0,
        message: envelope && envelope.message || data.description || '',
      });
    } catch (err) {
      console.error('[QishuiLoginCheckResult]', err);
      sendJSON(res, {
        provider: 'qishui',
        loggedIn: false,
        status: 'error',
        error: 'QISHUI_QR_CHECK_RESULT_FAILED',
        message: err && err.message || '汽水音乐签名检查结果处理失败',
      }, 500);
    }
    return;
  }
    ...
    try {
      ...
      if (!qishuiCookieHasLogin(normalized)) {
        sendJSON(res, {
          ...
          message: '汽水音乐 cookie 缺少有效登录票据(sessionid/uid_tt/sid_tt)',
        }, 400);
        return;
      }
  ...
  // ---------- 封面代理 (带 CORS 头, 给 canvas 提取像素用, Android 带本地文件缓存) ----------
  if (pn === '/api/cover') {
    try {
      ...
      // Android: 本地文件缓存(避免重复下载封面,减少流量)
      var coverCacheDir = process.env.MINERADIO_COVER_CACHE_DIR;
      var coverCacheFile = null;
      if (coverCacheDir && IS_ANDROID_PLATFORM) {
        try {
          if (!fs.existsSync(coverCacheDir)) fs.mkdirSync(coverCacheDir, { recursive: true });
          // 用 URL 的 hash 作为文件名(简单去重)
          var cryptoMod = require('crypto');
          var coverHash = cryptoMod.createHash('md5').update(coverUrl).digest('hex').slice(0, 16);
          var urlExt = '.jpg';
          try { var m = coverUrl.match(/\.(png|jpe?g|webp|gif)(\?|$)/i); if (m) urlExt = '.' + m[1].toLowerCase().replace('jpeg', 'jpg'); } catch (e) {}
          coverCacheFile = path.join(coverCacheDir, coverHash + urlExt);
          // 如果本地缓存存在,直接返回
          if (fs.existsSync(coverCacheFile)) {
            var stat = fs.statSync(coverCacheFile);
            var ext = path.extname(coverCacheFile).toLowerCase();
            var mimeMap = { '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.png': 'image/png', '.webp': 'image/webp', '.gif': 'image/gif' };
            var cachedCt = mimeMap[ext] || 'image/jpeg';
            res.writeHead(200, {
              'Content-Type': cachedCt,
            ...
            var readStream = fs.createReadStream(coverCacheFile);
            readStream.pipe(res);
            return;
          }
        } catch (e) { /* 缓存失败,继续走原始流程 */ }
      }
      ...
      if (coverCacheFile) {
        try { writeStream = fs.createWriteStream(coverCacheFile); } catch (e) { writeStream = null; }
      }
      ...
      while (true) {
        ...
        if (c.done) break;
        res.write(c.value);
        if (writeStream) { try { writeStream.write(c.value); } catch (e) {} }
      }
      if (writeStream) { try { writeStream.end(); } catch (e) {} }
      res.end();
